...
import {
  isTelemetryPrivacyDecision,
  type TelemetryPrivacyDecision,
  type TelemetryPrivacyPreference,
...
type PrivacyIpcHandlers = {
  ...
  ) => TelemetryPrivacyPreference | Promise<TelemetryPrivacyPreference>;
};
...
export function registerPrivacyIpcHandlers(
  ...
  registrar.handle("privacy:telemetry:set", (_event, preference) => {
    if (!isTelemetryPrivacyDecision(preference))
      throw new TypeError("invalid telemetry privacy preference");
