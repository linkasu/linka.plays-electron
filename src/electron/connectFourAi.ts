...
type ConnectFourAiAddon = {
  findBestMove: (
    ...
    timeLimitMs: number,
    threads: number,
  ) => Promise<Omit<ConnectFourAiResult, "source">>;
  findBestReversiMove?: (
    board: string,
    player: "R" | "Y",
    depth: number,
...
function addonCandidates() {
  ...
  if (app.isPackaged)
    candidates.push(
      join(process.resourcesPath, "app.asar.unpacked", "build", "Release", binaryName),
    );
...
function normalizeBoard(board: unknown) {
  ...
  return Array.from(board).every((cell) => cell === "." || cell === "R" || cell === "Y")
    ? board
    : undefined;
}
...
function normalizeCheckersBoard(board: unknown) {
  ...
  return Array.from(board).every(
    (cell) => cell === "." || cell === "g" || cell === "G" || cell === "b" || cell === "B",
  )
    ? board
    : undefined;
}
...
export function registerConnectFourAiHandlers() {
  ipcMain.handle(
    ...
    async (
      ...
      if (!addon || !board)
        return {
          ok: false,
          error: addon ? "invalid-board" : "native-addon-unavailable",
      ...
      try {
        ...
        return {
          ...result,
          ok: result.ok && result.column >= 0 && result.column < 7,
  ...
  ipcMain.handle(
    "reversi-light:best-move",
    async (
      _event,
      payload: { board?: unknown; player?: unknown; depth?: unknown; timeLimitMs?: unknown } = {},
    ) => {
      const addon = loadAddon();
      const board = normalizeReversiBoard(payload.board);
      if (!addon?.findBestReversiMove || !board)
        return {
          ok: false,
          error: addon ? "invalid-board" : "native-addon-unavailable",
          source: "fallback" as const,
      ...
      try {
        ...
        return {
          ...result,
          ok: result.ok && result.move >= 0 && result.move < 16,
          source: "native" as const,
        };
      } catch (error) {
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (!addon?.findBestCheckersMove || !board)
        return {
          ok: false,
          error: addon ? "invalid-board" : "native-addon-unavailable",
          source: "fallback" as const,
      ...
      const forcedFrom =
        typeof payload.forcedFrom === "number" && Number.isFinite(payload.forcedFrom)
          ? Math.floor(payload.forcedFrom)
          : -1;

      try {
        const result = await addon.findBestCheckersMove(
          board,
          side,
          depth,
          timeLimitMs,
        ...
        return {
          ...result,
          ok: result.ok && result.fromIndex >= 0 && result.toIndex >= 0,
          source: "native" as const,
        };
      } catch (error) {
  ...
  ipcMain.handle("chess-mini:legal-moves", async (_event, payload: { fen?: unknown } = {}) => {
    ...
    if (!addon?.chessLegalMoves || !fen)
      return {
        ok: false,
        error: addon ? "invalid-fen" : "native-addon-unavailable",
        source: "fallback" as const,
      };
  ...
  ipcMain.handle(
    "chess-mini:apply-move",
    async (
      _event,
      payload: { fen?: unknown; fromIndex?: unknown; toIndex?: unknown; promotion?: unknown } = {},
    ) => {
      const addon = loadAddon();
      const fen = normalizeChessFen(payload.fen);
      const fromIndex =
        typeof payload.fromIndex === "number" && Number.isFinite(payload.fromIndex)
          ? Math.floor(payload.fromIndex)
          : -1;
      const toIndex =
        typeof payload.toIndex === "number" && Number.isFinite(payload.toIndex)
          ? Math.floor(payload.toIndex)
          : -1;
      const promotion =
        typeof payload.promotion === "string" && payload.promotion.length > 0
          ? payload.promotion[0]
          : "q";
      if (
        !addon?.chessApplyMove ||
        !fen ||
        fromIndex < 0 ||
        fromIndex >= 64 ||
        toIndex < 0 ||
        ...
        return {
          ok: false,
          error: addon ? "invalid-move" : "native-addon-unavailable",
          source: "fallback" as const,
        };
  ...
  ipcMain.handle(
    "chess-mini:best-move",
    async (_event, payload: { fen?: unknown; depth?: unknown; timeLimitMs?: unknown } = {}) => {
      const addon = loadAddon();
      const fen = normalizeChessFen(payload.fen);
      if (!addon?.chessBestMove || !fen)
        return {
          ok: false,
          error: addon ? "invalid-fen" : "native-addon-unavailable",
          source: "fallback" as const,
