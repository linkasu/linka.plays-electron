...
describe("privacy IPC", () => {
  it("returns the current preference and accepts explicit decisions", async () => {
    ...
    const handlers = new Map(
      electronMock.handle.mock.calls.map(([channel, handler]) => [
        channel,
  ...
  it("rejects unknown as a renderer decision", () => {
    registerPrivacyIpcHandlers({
      getTelemetryPreference: () => "unknown",
      setTelemetryPreference: (preference) => preference,
    });
    const setHandler = electronMock.handle.mock.calls.find(
      ([channel]) => channel === "privacy:telemetry:set",
    )?.[1] as InvokeHandler;
