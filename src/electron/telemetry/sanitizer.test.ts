...
describe("telemetry sanitizer", () => {
  ...
  it.each([
    ...
    "message",
    "path",
  ])("rejects forbidden property %s", (key) => {
    expect(
      sanitizeRendererTelemetryEvent({
        eventName: "target_clicked",
        gameSessionId: sessionId,
  ...
  it("rejects unknown event payloads and raw error details", () => {
    expect(
      sanitizeRendererTelemetryEvent({ eventName: "custom", properties: { anything: true } }),
    ).toBeUndefined();
    expect(
      sanitizeRendererTelemetryEvent({
        eventName: "error",
  ...
  it("rejects unknown identifiers even when they use the safe alphabet", () => {
    expect(
      sanitizeRendererTelemetryEvent({
        ...
        properties: { page: "encoded-private-value" },
      }),
    ).toBeUndefined();
    expect(
      sanitizeRendererTelemetryEvent({
        ...
        gameSessionId: sessionId,
        properties: { gameId: "encoded-private-value" },
      }),
    ).toBeUndefined();
    expect(
      sanitizeRendererTelemetryEvent({
        eventName: "error",
        properties: { fingerprint: `sha256:${"a".repeat(64)}`, component: "renderer.private" },
      }),
    ).toBeUndefined();
  });
