...
async function createTelemetry() {
  ...
  return {
    userDataPath,
    telemetry: new MetricsTelemetry({
      enabled: true,
      metricsEndpoint: "https://metrics.example.test",
      identityEndpoint: "https://identity.example.test",
...
describe("MetricsTelemetry lifecycle", () => {
  it("serializes renderer summary before interruption finalization", async () => {
    ...
    expect(
      records.some(
        (record) =>
          record.kind === "event" &&
          "event_name" in record.payload &&
          record.payload.event_name === "game_session_interrupted",
      ),
    ).toBe(false);
  });
  ...
  it("shares one shutdown and writes one app close event", async () => {
    ...
    expect(
      records.filter(
        (record) =>
          record.kind === "summary" &&
          "session_type" in record.payload &&
          record.payload.session_type === "app",
      ),
    ).toHaveLength(0);
    expect(
      records.filter(
        (record) =>
          record.kind === "event" &&
          "event_name" in record.payload &&
          record.payload.event_name === "app_closed",
      ),
    ).toHaveLength(1);
  });
  ...
  it("does not create telemetry storage before initialization", async () => {
    ...
    await expect(stat(join(userDataPath, "telemetry-v1"))).rejects.toMatchObject({
      code: "ENOENT",
    });
  });
  ...
  it("splits a rejected oversized batch without discarding records", async () => {
    ...
    expect(fetchMock.mock.calls.map(([input]) => String(input))).toContain(
      "https://metrics.example.test/v2/batches",
    );
  ...
  it.each([400, 409, 422, 202])(
    "preserves the exact batch after ambiguous HTTP %s",
    async (status) => {
      vi.useFakeTimers();
      const bodies: string[] = [];
      const fetchMock = vi.fn(async (input: string | URL | Request, init?: RequestInit) => {
        ...
        if (String(input).endsWith("/v2/batches")) {
          ...
          return status === 202
            ? new Response(
                JSON.stringify({ batch_id: randomUUID(), accepted_records: 2, replayed: false }),
                { status },
              )
