...
const allGameItems: SpecialistGameItem[] = gameGroups.flatMap((group) =>
...
const selectedGameItems = computed(() =>
  selectedGroup.value
    ? allGameItems.filter((item) => item.category === selectedGroup.value?.category)
    : [],
);
          ...
          >
            <div>
              ...
              <p class="text-body-1 text-medium-emphasis mb-0">
                Каталог разбит на страницы: крупные цели, без скролла, с ключевыми параметрами
                занятия.
              </p>
            </div>
            <TobiiStatusBadge />
            ...
            <div class="d-flex flex-column flex-md-row align-md-center ga-4">
              <div class="flex-grow-1">
                ...
                <div class="text-body-2 text-medium-emphasis">
                  Применяется ко всем играм и самостоятельному меню
                </div>
              </div>
          ...
          <section v-else :aria-label="isSearchActive ? 'Результаты поиска' : 'Игры в папке'">
            ...
            >
              <div>
                ...
                <h2 class="text-h4 font-weight-bold mb-2">
                  {{ searchResults.length ? "Найденные игры" : "Ничего не найдено" }}
                </h2>
                <p class="text-body-1 text-medium-emphasis mb-0">
                  Запрос: {{ searchQuery.trim() }}
                </p>
              </div>
              <v-chip color="info" size="large" variant="tonal"
                >{{ searchResults.length }} игр · {{ pageLabel }}</v-chip
              >
            </div>
            <div
              v-else
                  ...
                  <div class="d-flex h-100 ga-4 text-left text-on-surface">
                    ...
                    <div class="d-flex flex-column min-w-0">
                      ...
                      <div class="d-flex flex-wrap ga-2">
                        <v-chip
                          :color="item.game.status === 'planned' ? 'default' : 'primary'"
                          size="small"
                          variant="tonal"
                          >{{ gameStatusLabels[item.game.status] }}</v-chip
                        >
                        <v-chip
                          v-if="isSearchActive"
                          color="secondary"
                        ...
                        <v-chip
                          ...
                          size="small"
                          variant="tonal"
                          >{{ item.game.recommendedSessionSeconds }} сек</v-chip
                        >
                        <v-chip
                          color="info"
            ...
            >
              <div class="d-flex flex-column justify-center h-100 ga-3">
                ...
                <p class="text-body-1 text-medium-emphasis mb-0">
                  Попробуйте другое слово: название, навык, категорию или статус.
                </p>
                <v-btn
                  class="align-self-start"
                  color="primary"
          ...
          <v-row class="mt-4" align="stretch" dense>
            ...
            <v-col cols="4">
              <v-btn
                ...
                class="nav-action"
                color="surface"
                :disabled="!selectedGroup && !isSearchActive"
