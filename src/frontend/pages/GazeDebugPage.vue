...
const progress = computed(() =>
  Math.min(
    100,
    ((targetIndex.value + Math.min(1, elapsedMs.value / pointDurationMs)) /
      targetFractions.length) *
      100,
  ),
);
...
const activePoint = computed<GazePoint>(
  () =>
    tobiiPoint.value ?? {
      x: target.value.x,
      y: target.value.y,
...
function buildPointReport(index: number): PointReport {
  ...
  const sum = valid.reduce((acc, sample) => ({ x: acc.x + sample.x, y: acc.y + sample.y }), {
    x: 0,
    y: 0,
  });
...
function buildReport(): DebugReport {
  ...
  const meanErrorPx = errors.length
    ? errors.reduce((sum, error) => sum + error, 0) / errors.length
    : undefined;
  const maxErrorPx = errors.length ? Math.max(...errors) : undefined;
  const ok =
    reports.length === targetFractions.length &&
    reports.every(
      (point) =>
        point.errorPx !== undefined && point.errorPx <= targetZone / 2 && point.validRatio >= 0.6,
    );
  return {
    ok,
    reason: ok
...
<template>
  <div class="gaze-debug">
    ...
    >
      <rect
        :x="target.x - targetZone / 2"
        :y="target.y - targetZone / 2"
        :width="targetZone"
        :height="targetZone"
        rx="10"
        class="target-zone"
      ...
      <circle
        v-if="activePoint.valid"
        :cx="activePoint.x"
        :cy="activePoint.y"
        r="12"
        class="gaze-dot"
        :class="`is-${activePoint.source}`"
      />
    </svg>
    ...
    <v-card class="debug-panel ma-4" rounded="xl" elevation="8">
      ...
      <v-card-text>
        <div class="text-body-2 text-medium-emphasis mb-3">
          Смотрите на девять мишеней по очереди. Каждая точка измеряется 1,6 секунды; мышь в
          результат не попадает.
        </div>
        <v-alert v-if="copyMessage && !reportDialog" class="mb-3" type="warning" variant="tonal">{{
          copyMessage
        }}</v-alert>
        <v-progress-linear
          class="mb-3"
        ...
        <v-row dense>
          <v-col cols="6">
            ...
            <div class="text-body-2">
              {{ formatNumber(activePoint.x) }}, {{ formatNumber(activePoint.y) }} /
              {{ activePoint.source }} / {{ activePoint.valid ? "valid" : "invalid" }}
            </div>
          </v-col>
          <v-col cols="6">
            <div class="text-caption text-medium-emphasis">Смещение сейчас</div>
            <div class="text-body-2">
              {{ formatNumber(liveOffset.x) }}, {{ formatNumber(liveOffset.y) }}
            </div>
          </v-col>
          <v-col cols="6">
          ...
          <v-col cols="6">
            ...
            <div class="text-body-2">
              {{ targetIndex + 1 }} / {{ targetFractions.length }}: {{ formatNumber(target.x) }},
          ...
          <v-col cols="6">
            ...
            <div class="text-body-2">
              {{
                tobiiPoint
                  ? `${formatNumber(tobiiPoint.x)}, ${formatNumber(tobiiPoint.y)} / ${tobiiPoint.valid ? "valid" : "invalid"}`
                  : "нет данных"
              }}
            </div>
          </v-col>
      ...
      <v-card-actions class="pa-4 pt-0 flex-wrap ga-2">
        <v-btn color="primary" :disabled="running || !hasFreshTobii" @click="startMeasurement"
          >Проверить 9 точек</v-btn
        >
    ...
    <v-dialog v-model="reportDialog" max-width="min(44rem, 92vw)">
      <v-card rounded="xl">
        <v-card-title>{{
          report?.ok ? "Замер готов" : "Нужно отправить разработчику"
        }}</v-card-title>
        <v-card-text>
          ...
          <v-row dense>
            ...
            <v-col cols="12" sm="6">
              ...
              <div class="text-body-2">
                {{ report?.viewport.width }} x {{ report?.viewport.height }} / DPR
                {{ report?.viewport.devicePixelRatio }}
              </div>
            </v-col>
          </v-row>
          ...
          <v-textarea
            ...
            variant="outlined"
          />
          <v-alert v-if="copyMessage" class="mt-2" type="success" variant="tonal">{{
            copyMessage
          }}</v-alert>
        </v-card-text>
        <v-card-actions class="pa-4 pt-0 flex-wrap ga-2">
...
.gaze-debug {
  ...
  background:
    radial-gradient(
      circle at 50% 50%,
      rgb(255 244 210 / 72%),
      rgb(202 232 244 / 88%) 42%,
      rgb(157 207 230 / 96%) 100%
    ),
