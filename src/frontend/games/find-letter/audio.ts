...
export function warmFindLetterAudio(enabled: boolean) {
  warmSoftPiano(enabled, notesToLoad);
}

export function playFindLetterSuccessMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, successMelody);
}

export function playFindLetterMistakeMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, mistakeMelody);
}

export function disposeFindLetterAudio() {
  disposeSoftPiano();
}
