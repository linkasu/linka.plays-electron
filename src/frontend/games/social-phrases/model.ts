...
export const socialPhrasesInstruction =
  "Посмотри на ситуацию и выбери, что можно сказать. Если тебе нужна помощь, ты не хочешь или хочешь остановиться, выбери такую карточку: это тоже важный ответ.";
  ...
  },
  {
    ...
    choices: [
      ...
      },
      {
        ...
        iconColor: "amber-darken-4",
        text: "Помоги мне попросить воду.",
        accepted: true,
      },
    ],
    ...
    choices: [
      {
        ...
        iconColor: "green-darken-3",
        text: "Спасибо, что помог открыть коробку.",
        accepted: true,
      },
      {
        id: "thanks-book",
      ...
      },
      {
        ...
        color: "amber-lighten-5",
        iconColor: "amber-darken-4",
        text: "Помоги мне сказать спасибо.",
        accepted: true,
      },
    ],
...
function buildSocialPhraseRound(
  scene: SocialPhraseScene,
  roundIndex: number,
  random = Math.random,
): SocialPhraseRound {
  ...
  const expectedChoice = choices.find(
    (choice) => choice.kind === scene.expectedKind && choice.accepted,
  );
...
export function createSocialPhraseDeck(random = Math.random): SocialPhraseRound[] {
  return shuffleItems(socialPhraseScenes, random).map((scene, index) =>
    buildSocialPhraseRound(scene, index + 1, random),
  );
}
...
export function evaluateSocialPhraseChoice(
  round: SocialPhraseRound,
  choice: SocialPhraseChoice,
): SocialPhraseEvaluation {
  ...
  return {
    ...
    feedback: accepted
      ...
      : "Эта фраза прозвучала. Можно выбрать другую фразу или попросить помощь.",
    isCorrect: accepted,
    noFail: true,
    functional,
...
export function validateSocialPhraseScenes() {
  ...
  for (const scene of socialPhraseScenes) {
    ...
    if (scene.choices.length < 3 || scene.choices.length > 4)
      errors.push(`${scene.id}: expected 3 or 4 choices`);
    if (!scene.choices.some((choice) => choice.kind === scene.expectedKind && choice.accepted))
      errors.push(`${scene.id}: no expected choice`);
    if (
      scene.choices.some(
        (choice) => choice.kind !== "functional" && choice.kind !== scene.expectedKind,
      )
    )
      errors.push(`${scene.id}: mixed semantic roles`);
    if (
      scene.choices.some(
        (choice) => choice.kind === "functional" && (!choice.function || !choice.accepted),
      )
    )
      errors.push(`${scene.id}: invalid functional choice`);
    if (new Set(scene.choices.map((choice) => choice.id)).size !== scene.choices.length)
      errors.push(`${scene.id}: duplicate choice id`);
    if (new Set(scene.choices.map((choice) => choice.icon)).size !== scene.choices.length)
      errors.push(`${scene.id}: duplicate choice icon`);
    if (new Set(scene.choices.map((choice) => choice.color)).size !== scene.choices.length)
      errors.push(`${scene.id}: duplicate choice color`);
    if (scene.choices.some((choice) => !choice.text || !/[.!?]$/.test(choice.text)))
      errors.push(`${scene.id}: incomplete phrase`);
  }
  return errors;
}
