...
describe("social-phrases model", () => {
  ...
  it("builds four visually distinct situations with three or four visual options", () => {
    ...
    expect(new Set(socialPhraseScenes.map((scene) => scene.sceneIcon))).toHaveLength(
      socialPhraseScenes.length,
    );
    expect(new Set(socialPhraseScenes.map((scene) => scene.sceneColor))).toHaveLength(
      socialPhraseScenes.length,
    );
    ...
    for (const scene of socialPhraseScenes) {
      ...
      expect(new Set(scene.choices.map((choice) => choice.icon))).toHaveLength(
        scene.choices.length,
      );
      expect(new Set(scene.choices.map((choice) => choice.color))).toHaveLength(
        scene.choices.length,
      );
    }
  });
  ...
  it("does not mix semantic roles among situation answers", () => {
    for (const scene of socialPhraseScenes) {
      const situationKinds = new Set(
        scene.choices.filter((choice) => choice.kind !== "functional").map((choice) => choice.kind),
      );
  ...
  it("deals a complete unique scene deck and copies its options", () => {
    ...
    for (const round of deck) {
      expect(round.choices).not.toBe(
        socialPhraseScenes.find((scene) => scene.id === round.id)?.choices,
      );
  ...
  it("counts refusal, help, and stop as no-fail communication", () => {
    const deck = createSocialPhraseDeck(() => 0.37);
    const functionalChoices = deck.flatMap((round) =>
      round.choices
        .filter((choice) => choice.kind === "functional")
        .map((choice) => ({ round, choice })),
    );
