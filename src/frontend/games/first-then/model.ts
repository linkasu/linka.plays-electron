...
export function createFirstThenTimeline(
  pair: FirstThenPair,
  revealedPhases: FirstThenPhase[],
): FirstThenTimelineItem[] {
...
export function createFirstThenPairOrder(random = Math.random) {
  return shuffleItems(
    firstThenPairs.map((_, index) => index),
    random,
  );
}

export function generateFirstThenRound(
  roundIndex = 1,
  phase: FirstThenPhase = "first",
  options: FirstThenRoundOptions = {},
): FirstThenRound {
  if (firstThenPairs.length === 0)
    throw new Error("Недостаточно пар действий для игры Сначала-потом.");

  const pairOrder = options.pairOrder?.length
    ? options.pairOrder
    : firstThenPairs.map((_, index) => index);
