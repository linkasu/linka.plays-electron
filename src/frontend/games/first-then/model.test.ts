...
describe("first-then model", () => {
  it("creates a first phase round with both actions", () => {
    ...
    const round = generateFirstThenRound(1, "first", {
      choiceOrder: [firstPair.first.id, firstPair.then.id],
    });
  ...
  it("keeps a provided choice order stable across phases", () => {
    ...
    expect(generateFirstThenRound(1, "first", { choiceOrder }).choices).toEqual([
      firstPair.then,
      firstPair.first,
    ]);
    expect(generateFirstThenRound(1, "then", { choiceOrder }).choices).toEqual([
      firstPair.then,
      firstPair.first,
    ]);
  });
  ...
  it("does not leak the correct order through choice copy", () => {
    const round = generateFirstThenRound(1, "first", {
      choiceOrder: [firstThenPairs[0].first.id, firstThenPairs[0].then.id],
    });
    const choiceCopy = round.choices
      .map((choice) => `${choice.title} ${choice.emoji}`)
      .join(" ")
  ...
  it("keeps the timeline empty until correct phases are revealed", () => {
    ...
    expect(createFirstThenTimeline(pair, []).map((item) => item.action)).toEqual([
      undefined,
      undefined,
    ]);
    expect(createFirstThenTimeline(pair, ["first"]).map((item) => item.action)).toEqual([
      pair.first,
      undefined,
    ]);
    expect(createFirstThenTimeline(pair, ["first", "then"]).map((item) => item.action)).toEqual([
      pair.first,
      pair.then,
    ]);
  });
