...
export function leavesWindSceneBounds(
...
export function leavesWindTargetRadius(
  viewportWidth: number,
  viewportHeight: number,
  targetScale: number,
) {
  ...
  const availableRadius = Math.max(
    1,
    Math.min((scene.right - scene.left) / 2, (scene.bottom - scene.top) / 2),
  );
  return Math.min(scaledRadius, availableRadius);
}

export function leavesWindTargetBounds(
  viewportWidth: number,
  viewportHeight: number,
  targetRadius: number,
): LeavesWindBounds {
...
export function clampLeavesWindPoint(
  point: LeavesWindPoint,
...
export function leavesWindMotionPoint(
  anchor: LeavesWindPoint,
  bounds: LeavesWindBounds,
...
export function isLeavesWindTargetHit(
  point: LeavesWindPoint,
  target: LeavesWindPoint,
  radius: number,
) {
