...
import {
  generatePizzaFractionsRound,
  type PizzaFractionChoice,
  type PizzaFractionsRound,
} from "./model";
...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("pizza-fractions", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  ...
  soundEnabled,
  warmAssetIds: [
    "pizza-fractions.prompt.half",
    "pizza-fractions.prompt.quarter",
    "pizza-fractions.prompt.whole",
    "pizza-fractions.correct",
    "pizza-fractions.mistake",
    "pizza-fractions.complete",
  ],
});
  ...
  if (index === round.value.correctIndex) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["pizza-fractions.correct", "pizza-fractions.complete"]
        : ["pizza-fractions.correct"],
      80,
      170,
    );
        ...
        <v-col cols="12" xl="11">
          <v-card class="pa-4 pa-md-6" rounded="xl" elevation="8">
            <div class="text-overline text-secondary text-center mb-2">
              Половина, четверть и целое
            </div>
            <h1 class="text-h3 text-md-h2 font-weight-bold text-center mb-3">{{ round.prompt }}</h1>
            <v-alert
              class="mb-5 text-body-1 font-weight-bold"
              :color="mistakenChoiceId ? 'secondary' : 'primary'"
              :icon="mistakenChoiceId ? 'mdi-heart-outline' : 'mdi-lightbulb-outline'"
              rounded="xl"
              variant="tonal"
            >
              {{ feedbackText }}
                ...
                >
                  <template #default>
                    <div
                      :class="[
                        'pizza-choice',
                        { 'pizza-choice--mistake': mistakenChoiceId === choice.id },
                      ]"
                    >
                      <svg
                        class="pizza-plate"
                      ...
                      >
                        ...
                        <g class="pizza-plate__slices">
                          <path
                            v-for="slice in pizzaSlices"
                            :key="slice.id"
                            :class="[
                              'pizza-plate__slice',
                              { 'pizza-plate__slice--filled': isSliceFilled(choice, slice.id) },
                            ]"
                            :d="slice.path"
                          />
                        </g>
                        ...
                        <circle
                          v-if="choice.id !== 'whole'"
                          class="pizza-plate__focus"
                          cx="50"
                          cy="50"
                          r="42"
                        />
                      </svg>
                      <div class="pizza-choice__label text-h4 text-md-h3 font-weight-bold mt-4">
                        {{ choice.label }}
                      </div>
                      <v-chip
                        class="pizza-choice__chip mt-2"
                        color="deep-purple-darken-3"
                        size="large"
                        variant="flat"
                        >{{ choice.shortLabel }}</v-chip
                      >
                      <div class="pizza-choice__helper text-body-1 text-medium-emphasis mt-3">
                        {{ choice.helperText }}
                      </div>
