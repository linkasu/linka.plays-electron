...
export const dailyScheduleSteps: ScheduleCard[] = [
  ...
  },
  {
    ...
    emoji: "👕",
    color: "#b39ddb",
    hint: "После завтрака надеваем одежду.",
  },
  {
    id: "therapy",
    title: "Занятие",
  ...
  },
  {
    ...
    emoji: "🍲",
    color: "#a5d6a7",
    hint: "После занятия время обеда.",
  },
  {
    id: "play",
    title: "Игра",
...
export function createScheduleCards(): ScheduleCard[] {
  return cardChoiceOrder
    .map((id) => dailyScheduleSteps.find((step) => step.id === id))
    .filter((step): step is ScheduleCard => Boolean(step));
}

export function createScheduleCandidates<T extends ScheduleCard>(
  cards: readonly T[],
  placedIds: readonly string[],
  maxVisible = cards.length,
): T[] {
  ...
  const expectedIndex = Math.max(
    0,
    available.findIndex((card) => card.id === expectedId),
  );
  const start = Math.min(
    Math.max(0, expectedIndex - Math.floor(limit / 2)),
