...
const { session, durationMs, metrics, recommendation, pauseSession, resumeSession, recordSuccess, recordMistake, recordHint, startSession } = useGameSession("what-first", {
  maxSteps: 8,
  dwellMs: 1300,
  sessionSeconds: 125
        ...
        <v-col cols="12" lg="10" xl="9">
          <v-card class="pa-5 pa-md-8" rounded="xl" elevation="8">
            ...
            <div class="text-center mb-6">
              <v-chip class="mb-4" color="primary" size="large" variant="tonal">{{ round.scene.title }}</v-chip>
              ...
              <div class="text-h6 text-md-h5 text-medium-emphasis">{{ round.scene.context }}</div>
            </div>
            ...
            <v-row>
              <v-col v-for="choice in round.choices" :key="choice.id" cols="12" md="6">
                <GameDwellButton :target-id="choiceTargetId(choice)" :disabled="session.status !== 'running' || isChangingRound" :dwell-ms="session.settings.dwellMs" :min-height="230" :color="choiceColor(choice)" @select="choose(choice)">
                  <template #default>
                    <div class="choice-emoji emoji-glyph">{{ choice.emoji }}</div>
                    <div class="text-h3 text-md-h2 font-weight-bold mb-2">{{ choice.title }}</div>
                    <v-chip class="mb-3" color="primary" size="large" variant="tonal">{{ choice.aacLabel }}</v-chip>
