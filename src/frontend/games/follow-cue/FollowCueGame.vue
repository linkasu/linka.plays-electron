...
import {
  ...
  playFollowCueMistakeMelody,
  playFollowCueSuccessMelody,
  warmFollowCueAudio,
} from "./audio";
...
const cueSlots: CueSlot[] = [
  ...
  { id: "top-right", label: "сверху справа", className: "cue-target--top-right", rotationDeg: -45 },
  {
    id: "bottom-left",
    label: "снизу слева",
    className: "cue-target--bottom-left",
    rotationDeg: 135,
  },
  {
    id: "bottom-right",
    label: "снизу справа",
    className: "cue-target--bottom-right",
    rotationDeg: 45,
  },
];
...
async function answer(choice: CueChoice) {
  ...
  if (choice.isTarget) {
    ...
    recordSuccess({
      ...
      targetId,
      answerId: choice.id,
      expected: round.value.target.slot.label,
      actual: choice.slot.label,
      isCorrect: true,
    });
    cueStrength.value = 0;
  ...
  recordHint({
    ...
    reason: "wrong-cue-target",
    strength: cueStrength.value,
  });
        ...
        <v-col cols="12" lg="10" xl="9">
          <v-card class="follow-cue-card pa-3 pa-md-5" rounded="xl" elevation="8">
            ...
            <div
              ...
              :style="cueStyle"
              role="list"
              aria-label="Крупные цели для выбора"
            >
              ...
              <GameDwellButton
                ...
                :class="[
                  'cue-target',
                  choice.slot.className,
                  {
                    'cue-target--hinted': isFollowCueHintVisible(choice.isTarget, cueStrength),
                    'cue-target--mistake': lastMistakeId === choice.id,
                  },
                ]"
              ...
              >
                <template #default="{ active, progress }">
                  <v-icon
                    ...
                    :color="
                      isFollowCueHintVisible(choice.isTarget, cueStrength)
                        ? undefined
                        : choice.item.color
                    "
                  />
                  <div class="text-body-1 text-md-h6 font-weight-bold mt-2">
                    {{ choice.item.label }}
                  </div>
                  <v-chip
                    v-if="
                      isFollowCueHintVisible(choice.isTarget, cueStrength) &&
                      (cueStrength > 1 || (active && progress > 0.78))
                    "
                    class="mt-3"
                    color="primary"
                    variant="flat"
            ...
            <div class="cue-feedback-slot mt-2">
              <v-alert
                class="cue-feedback text-body-2"
                :class="{ 'cue-feedback--visible': cueStrength > 0 }"
                color="primary"
