...
export const buildBridgeSlots: BridgeSlot[] = [
  {
    id: "slot-support-left",
    label: "место левой опоры",
    kind: "support",
    acceptsPieceId: "support-left",
    supportedBy: ["riverbed"],
  },
  {
    id: "slot-support-center",
    label: "место средней опоры",
    kind: "support",
    acceptsPieceId: "support-center",
    supportedBy: ["riverbed"],
  },
  {
    id: "slot-support-right",
    label: "место правой опоры",
    kind: "support",
    acceptsPieceId: "support-right",
    supportedBy: ["riverbed"],
  },
  {
    id: "slot-support-far",
    label: "место дальней опоры",
    kind: "support",
    acceptsPieceId: "support-far",
    supportedBy: ["riverbed"],
  },
  {
    id: "slot-plank-one",
    label: "пролёт 1",
    kind: "plank",
    acceptsPieceId: "plank-one",
    supportedBy: ["bank-left", "support-left"],
  },
  {
    id: "slot-plank-two",
    label: "пролёт 2",
    kind: "plank",
    acceptsPieceId: "plank-two",
    supportedBy: ["support-left", "support-center"],
  },
  {
    id: "slot-plank-three",
    label: "пролёт 3",
    kind: "plank",
    acceptsPieceId: "plank-three",
    supportedBy: ["support-center", "support-right"],
  },
  {
    id: "slot-plank-four",
    label: "пролёт 4",
    kind: "plank",
    acceptsPieceId: "plank-four",
    supportedBy: ["support-right", "support-far"],
  },
  {
    id: "slot-plank-five",
    label: "пролёт 5",
    kind: "plank",
    acceptsPieceId: "plank-five",
    supportedBy: ["support-far", "bank-right"],
  },
];
...
export function createBuildBridgeState(
  placements: readonly BridgePlacement[] = [],
): BuildBridgeState {
...
export function hasBridgeSupportAt(supportId: string, placements: readonly BridgePlacement[]) {
  return (
    permanentSupports.has(supportId) || supportIdsFromPlacements(placements).includes(supportId)
  );
}

export function canPlaceBridgePieceAtSlot(
  pieceId: string,
  slotId: string,
  placementsOrPlacedIds: readonly BridgePlacement[] | readonly string[],
) {
  const piece = bridgePieceById(pieceId);
  const slot = bridgeSlotById(slotId);
  if (!piece || !slot) return false;
  const placements = placementsOrPlacedIds.filter(
    (item): item is BridgePlacement =>
      typeof item === "object" && item !== null && "pieceId" in item && "slotId" in item,
  );
  const placedIds = placements.length
    ? placedPieceIds(placements)
    : (placementsOrPlacedIds as readonly string[]);
  ...
  if (piece.kind === "support")
    return slot.supportedBy.every((supportId) => hasBridgeSupport(supportId, placedIds));
  return slot.supportedBy.every((supportId) =>
    placements.length
      ? hasBridgeSupportAt(supportId, placements)
      : hasBridgeSupport(supportId, placedIds),
  );
}
...
export function advanceBuildBridge(state: BuildBridgeState, slotId: string): BuildBridgeTransition {
  ...
  const slot = bridgeSlotById(slotId);
  if (!piece || !slot || slot.kind !== piece.kind || isBridgeSlotOccupied(slotId, state.placements))
    return { kind: "ignored", state };
  if (!canPlaceBridgePieceAtSlot(piece.id, slot.id, state.placements))
    return { kind: "soft-error", state, piece, slot };
