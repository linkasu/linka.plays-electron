...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("build-bridge", {
...
function computeLayout(): Layout {
  ...
  const cards: Rect = {
    ...
    y: cardsY,
    width: panel.width - 48,
    height: Math.max(180, panel.y + panel.height - cardsY - 18),
  };
...
function targetPayload(
  ...
  now: number,
  progress: number,
  reason?: "left" | "invalid-gaze" | "disabled",
) {
  return {
    targetId,
    at: Date.now(),
...
function resetDwell(now: number, reason?: "left" | "invalid-gaze" | "disabled") {
  if (activeTargetId && reason)
    recordEvent("target-cancel", targetPayload(activeTargetId, now, activeProgress, reason));
  activeTargetId = "";
  activeStartedAt = 0;
  activeProgress = 0;
}
...
function hitTestTarget(point: Point) {
  ...
  const resolved = resolveGazeTarget(
    targets.map((target) => ({
  ...
  return resolved
    ? targets.find((target) => bridgeSlotTargetId(target.slot) === resolved.id)
    : undefined;
}
...
function spawnFallingPiece(piece: BridgePiece, source: Rect) {
  ...
  fallingPieces.push({
    ...
    width:
      piece.kind === "support"
        ? Math.min(72, source.width * 0.34)
        : Math.min(118, source.width * 0.5),
    height:
      piece.kind === "support"
        ? Math.min(104, source.height * 0.56)
        : Math.min(38, source.height * 0.2),
...
function placeCurrentPiece(target: SlotTarget) {
  ...
  if (outcome.kind === "placed") {
    ...
    recordSuccess({
      targetId,
      answerId: outcome.piece.id,
      slotId: target.slot.id,
      action: "placed-piece",
    });
    const nextPiece = currentBridgePiece(outcome.state);
    message.value = nextPiece
      ? `Деталь держится. Теперь поставь ${nextPiece.label}.`
      : "Мост готов. Все детали нашли опору.";
    void bridgeFeedback.playSuccess(session.settings.sound);
    if (outcome.state.phase === "complete" && session.status === "running") {
      void playAudio(["build-bridge.correct", "build-bridge.complete"], 80).then(() =>
  ...
  recordMistake({
    targetId,
    expectedTargetId: expectedSlot ? bridgeSlotTargetId(expectedSlot) : undefined,
    answerId: outcome.piece.id,
    slotId: target.slot.id,
    action: "piece-fell",
    isCorrect: false,
  ...
  recordEvent("hint", {
    ...
    slotId: target.slot.id,
    supportedBy: target.slot.supportedBy,
  });
...
function updatePhysics(delta: number) {
  ...
  for (let index = fallingPieces.length - 1; index >= 0; index -= 1) {
    ...
    if (body.x < -160 || body.x > width.value + 160 || body.age > 5.2)
...
function drawHeader(ctx: CanvasRenderingContext2D, layout: Layout) {
  ...
  if (height.value >= 680)
    drawText(ctx, "ПОСЛЕДОВАТЕЛЬНОСТЬ ДЕТАЛЕЙ", centerX, layout.header.y + 18, {
      size: 13,
      weight: 700,
      color: "#8e76c7",
    });
  drawText(ctx, "Построй мост", centerX, layout.header.y + (height.value < 680 ? 22 : 56), {
    size: clamp(width.value * 0.034, 28, 54),
    weight: 800,
    color: "#40524f",
  });
  if (height.value >= 680)
    drawText(ctx, message.value, centerX, layout.header.y + layout.header.height - 16, {
      size: clamp(width.value * 0.012, 15, 22),
      weight: 500,
      color: "#768481",
    });
}
...
function bridgeGeometry(rect: Rect): BridgeGeometry {
  ...
  return {
    ...
    rightBank: {
      x: river.x + river.width * 0.8,
      y: bankTopY,
      width: river.width * 0.2,
      height: bankHeight + 12,
    },
    bankTopY,
    ...
    segmentWidth,
    supportXs: [
      deckStartX + segmentWidth,
      deckStartX + segmentWidth * 2,
      deckStartX + segmentWidth * 3,
      deckStartX + segmentWidth * 4,
    ],
  };
}
...
function drawWater(ctx: CanvasRenderingContext2D, rect: Rect) {
  ...
  ctx.fillRect(
    geometry.leftBank.x,
    geometry.leftBank.y,
    geometry.leftBank.width,
    geometry.leftBank.height,
  );
  ctx.fillRect(
    geometry.rightBank.x,
    geometry.rightBank.y,
    geometry.rightBank.width,
    geometry.rightBank.height,
  );
...
function slotRect(bridge: Rect, slot: BridgeSlot): Rect {
  ...
  if (slot.kind === "support") {
    ...
    return {
      x: x - supportWidth * 0.5,
      y: topY,
      width: supportWidth,
      height: geometry.riverBottom - topY,
    };
  }
  ...
  return {
    ...
    width:
      ...
      (index > 0 ? overlap * 0.5 : 0) +
      (index < planks.length - 1 ? overlap * 0.5 : 0),
...
function drawBridgeSupport(ctx: CanvasRenderingContext2D, rect: Rect, color = "#607d8b") {
  ...
  fillRoundedRect(
    ctx,
    { x: centerX - capWidth * 0.5, y: topY - 8, width: capWidth, height: 12 },
    6,
    "rgb(67 86 92 / 82%)",
  );
  ctx.fillStyle = color;
  for (const offset of [-0.24, 0, 0.24]) {
    fillRoundedRect(
      ctx,
      {
        x: centerX + offset * rect.width - pillarWidth * 0.5,
        y: topY + 5,
        width: pillarWidth,
        height: Math.max(22, bedY - topY - 31),
      },
      5,
      color,
    );
  ...
  fillRoundedRect(
    ctx,
    { x: centerX - capWidth * 0.44, y: bedY - 30, width: capWidth * 0.88, height: 12 },
    6,
    "rgb(67 86 92 / 78%)",
  );
  fillRoundedRect(
    ctx,
    { x: centerX - capWidth * 0.62, y: bedY - 14, width: capWidth * 1.24, height: 18 },
    9,
    "rgb(82 94 86 / 58%)",
  );
  ctx.restore();
...
function drawBridgeFootings(ctx: CanvasRenderingContext2D, rect: Rect) {
  ...
  for (const slot of buildBridgeSlots.filter((item) => item.kind === "support")) {
    ...
    fillRoundedRect(
      ctx,
      {
        x: centerX - ghost.width * 0.62,
        y: geometry.riverBottom - 18,
        width: ghost.width * 1.24,
        height: 20,
      },
      10,
      "rgb(70 86 80 / 34%)",
...
function drawFinishedMessage(ctx: CanvasRenderingContext2D, rect: Rect) {
  ...
  const banner: Rect = {
    x: rect.x + rect.width * 0.5 - bannerWidth * 0.5,
    y: rect.y + 18,
    width: bannerWidth,
    height: bannerHeight,
  };
  ...
  drawText(ctx, "Мост готов", banner.x + banner.width * 0.5, banner.y + banner.height * 0.42, {
    size: clamp(rect.width * 0.018, 20, 30),
    weight: 800,
    color: "#3f5f51",
  });
  drawText(
    ctx,
    "Все доски нашли опору",
    banner.x + banner.width * 0.5,
    banner.y + banner.height * 0.74,
    { size: clamp(rect.width * 0.011, 13, 17), weight: 600, color: "#5b756d" },
  );
}
...
function drawBridgeScene(ctx: CanvasRenderingContext2D, rect: Rect) {
  ...
  const bridgeHint =
    session.status === "finished"
      ? "Мост собран"
      : activePiece.value
        ? `Поставь: ${activePiece.value.label}`
        : "Мост собран";
  drawText(ctx, bridgeHint, rect.x + rect.width * 0.5, rect.y + rect.height - 24, {
    size: clamp(rect.width * 0.018, 13, 20),
    weight: 600,
    color: "#5d7775",
  });
...
function drawPieceIcon(
  ...
  drawPlank(
    ctx,
    {
      x: center.x - scale * 0.58,
      y: center.y - scale * 0.12,
      width: scale * 1.16,
      height: scale * 0.24,
    },
    piece.color,
...
function drawCurrentPieceCard(ctx: CanvasRenderingContext2D, rect: Rect) {
  ...
  drawPieceIcon(
    ctx,
    piece,
    { x: rect.x + rect.width * 0.5, y: rect.y + rect.height * 0.35 },
    iconScale,
  );
  drawText(ctx, piece.label, rect.x + rect.width * 0.5, rect.y + rect.height * 0.68, {
    size: clamp(rect.height * 0.15, 20, 34),
    weight: 800,
  });
  drawText(
    ctx,
    `деталь ${piece.order} из ${buildBridgeMaxSteps}: выбери место`,
    rect.x + rect.width * 0.5,
    rect.y + rect.height * 0.84,
    { size: clamp(rect.height * 0.075, 12, 17), weight: 500, color: "#52605d" },
  );
}
...
function drawFallingPieces(ctx: CanvasRenderingContext2D) {
  for (const body of fallingPieces) {
    ...
    if (body.piece.kind === "support")
      drawSupport(ctx, 0, body.height * 0.5, body.width * 0.72, body.piece.color);
    else
      drawPlank(
        ctx,
        { x: -body.width * 0.5, y: -body.height * 0.5, width: body.width, height: body.height },
        body.piece.color,
      );
    ctx.restore();
  }
