...
import {
  ...
  playMusicalPathComplete,
  playMusicalPathMistake,
  playMusicalPathNote,
  warmMusicalPathAudio,
} from "./audio";
import {
...
const {
  ...
  recordMistake,
  startSession,
} = useGameSessionFor("musical-path", {
  maxSteps: 8,
  overrides: {
    preset: "gentle",
    targetScale: 1.45,
...
function showSoftError(stone: MusicalPathStone) {
  ...
  feedbackMessage.value =
    "Этот камешек подождёт. Посмотри на подсвеченный камешек и попробуй ещё раз.";
  window.clearTimeout(errorTimer);
...
function chooseStone(stone: MusicalPathStone) {
  ...
  addSpark(stone);
  feedbackMessage.value =
    stone.order === session.maxSteps
      ? "Дорожка собрана. Мелодия завершилась."
      : `Верно: ${stone.note}. Теперь камешек ${stone.order + 1}.`;
  recordSuccess({
    roundId,
    targetId: targetId(stone),
    note: stone.note,
    ...
    <main class="path-stage" aria-label="Музыкальная дорожка: выбирай камешки по порядку от 1 до 8">
      ...
      >
        <template #default="{ active, progress }">
          <div
            class="path-stone"
            :class="{ 'path-stone--active': active, 'path-stone--done': stone.selected }"
            :style="stoneStyle(stone)"
          >
            ...
            <div
              class="stone-active-glow"
              :style="{ opacity: active ? 0.26 + progress * 0.36 : 0.18 }"
            />
          </div>
        </template>
      </GameDwellButton>
        ...
        <div class="d-flex align-center ga-3">
          ...
          <div>
            <div class="text-h6 font-weight-bold">
              {{ nextStone ? `Камешек ${nextStone.order}: ${nextStone.note}` : "Дорожка готова" }}
            </div>
...
.path-sky {
  background:
    radial-gradient(circle at 24% 24%, rgb(255 255 255 / 82%) 0 13%, transparent 28%),
    radial-gradient(circle at 78% 16%, rgb(255 243 210 / 58%) 0 10%, transparent 25%),
    radial-gradient(
      ellipse at 50% 85%,
      rgb(255 255 255 / 72%) 0%,
      rgb(255 255 255 / 26%) 35%,
      transparent 70%
    );
  block-size: 100%;
  inline-size: 100%;
...
.path-stone {
  ...
  background: radial-gradient(
    circle at 34% 26%,
    hsl(var(--stone-hue) 86% 95% / 0.96),
    hsl(var(--stone-hue) 58% 72% / 0.94) 43%,
    hsl(var(--stone-hue) 42% 52% / 0.98) 100%
  );
  border: 0.125rem solid hsl(var(--stone-hue) 80% 92% / 0.76);
  border-radius: 46% 54% 50% 50% / 52% 44% 56% 48%;
  box-shadow:
    0 1.5rem 2.5rem hsl(var(--stone-hue) 40% 32% / 0.18),
    inset -0.875rem -1rem 1.75rem hsl(var(--stone-hue) 36% 34% / 0.22),
    inset 1rem 1rem 1.625rem rgb(255 255 255 / 0.34);
...
.path-stone-target--next .path-stone {
  box-shadow:
    0 0 0 0.5rem hsl(var(--stone-hue) 82% 78% / 0.2),
    0 1.625rem 2.75rem hsl(var(--stone-hue) 40% 32% / 0.2),
    inset -0.875rem -1rem 1.75rem hsl(var(--stone-hue) 36% 34% / 0.2),
    inset 1rem 1rem 1.625rem rgb(255 255 255 / 0.36);
}
...
@media (min-width: 47.5625rem) and (max-width: 56.25rem),
  (min-width: 47.5625rem) and (max-height: 43.75rem) {
  .path-stone-target {
