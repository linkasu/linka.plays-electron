...
const shapePromptAssetIds = [
  "shapes.prompt.circle",
  "shapes.prompt.square",
  "shapes.prompt.triangle",
  "shapes.prompt.star",
];
const promptAudio = useGamePromptAudio({
  gameId: "shapes",
  soundEnabled,
  warmAssetIds: [...shapePromptAssetIds, "shapes.correct", "shapes.mistake", "shapes.complete"],
});
...
async function choose(choice: ShapeOption, index: number) {
  ...
  if (index === round.value.correctIndex) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess ? ["shapes.correct", "shapes.complete"] : ["shapes.correct"],
      80,
      170,
    );
      ...
      <v-row justify="center" no-gutters>
        <v-col cols="12" xl="10">
          <v-card class="shapes-card pa-4 pa-md-6" rounded="xl" elevation="8">
            ...
            <v-alert
              class="hint-alert mb-4"
              :class="{ 'hint-alert--visible': hintText }"
              color="primary"
              variant="tonal"
              rounded="xl"
              density="comfortable"
                ...
                >
                  <template #default>
                    <div
                      :class="[
                        'shape-wrap',
                        { 'shape-wrap--mistake': lastMistakeId === choice.id },
                      ]"
                      :aria-label="shapeView[choice.id].label"
                    >
                      <svg
                        class="shape-svg"
                        viewBox="0 0 120 120"
                      ...
                      <div class="text-h5 text-md-h4 font-weight-bold mt-2">
                        {{ shapeView[choice.id].label }}
                      </div>
                    </div>
                  </template>
                </GameDwellButton>
