...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("choose-emotion", {
  maxSteps: 8,
...
const helperText = computed(() => {
  ...
  if (lastChoiceId.value)
    return round.value.mode === "face"
      ? "Посмотри на лицо ещё раз и выбери другой ответ."
      : "Посмотри на ситуацию ещё раз и выбери другой ответ.";
  return round.value.mode === "face"
    ? "Выбери лицо с таким же выражением."
    : "Выбери подходящее лицо.";
});
  ...
  if (index === round.value.correctIndex) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? [`choose-emotion.emotion.${choice.id}`, "choose-emotion.complete"]
        : [`choose-emotion.emotion.${choice.id}`],
      80,
      170,
    );
    isSpeaking.value = false;
          ...
          <v-card class="emotion-card pa-4 pa-md-7" rounded="xl" elevation="8">
            <div v-if="selectingMode" class="text-center">
              ...
              <v-row justify="center">
                <v-col cols="12" sm="6">
                  <GameDwellButton
                    target-id="choose-emotion:mode:face"
                    :dwell-ms="session.settings.dwellMs"
                    min-height="12rem"
                    color="pink-lighten-5"
                    @select="startGame('face')"
                  >
                    <template #default>
                      <div class="d-flex flex-column align-center justify-center ga-3 pa-4">
                        ...
                        <div class="text-body-2 text-grey-darken-3">
                          Посмотри на выражение лица и выбери ответ
                        </div>
                      </div>
                    </template>
                  </GameDwellButton>
                ...
                <v-col cols="12" sm="6">
                  <GameDwellButton
                    target-id="choose-emotion:mode:situation"
                    :dwell-ms="session.settings.dwellMs"
                    min-height="12rem"
                    color="blue-lighten-5"
                    @select="startGame('situation')"
                  >
                    <template #default>
                      <div class="d-flex flex-column align-center justify-center ga-3 pa-4">
                        <div class="mode-emoji emoji-glyph" aria-hidden="true">🎁</div>
                        ...
                        <div class="text-body-2 text-grey-darken-3">
                          Посмотри на ситуацию и выбери лицо
                        </div>
                      </div>
                    </template>
                  </GameDwellButton>
            ...
            <template v-else>
              ...
              <p class="text-h6 text-md-h5 text-medium-emphasis text-center mb-2">
                {{ round.detail }}
              </p>
              ...
              <v-row class="choice-grid" justify="center" dense>
                ...
                >
                  <GameDwellButton
                    :class="{ 'success-choice': successChoiceId === choice.id }"
                    :target-id="choiceTargetId(choice.id)"
                    :disabled="session.status !== 'running' || pendingSelection || isSpeaking"
                    :dwell-ms="session.settings.dwellMs"
                    min-height="12.5rem"
                    :color="successChoiceId === choice.id ? 'green-lighten-4' : 'surface'"
                    @select="choose(choice, index)"
                  >
                    <template #default>
                      <div
                        :class="[
                          'emotion-choice',
                          { 'emotion-choice--last': lastChoiceId === choice.id },
                        ]"
                      >
