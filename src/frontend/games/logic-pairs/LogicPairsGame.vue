...
const helperText = computed(() => {
  if (hintedRoundId.value === round.value.roundId)
    return `Почти. ${round.value.explanation} Правильная карточка подсвечена.`;
...
async function choose(choice: LogicPairCard) {
  ...
  if (choice.id === round.value.pair.id) {
    ...
    recordSuccess({
      ...
      targetId,
      answerId: choice.id,
      expected: round.value.pair.label,
      actual: choice.label,
      relation: round.value.relation,
      isCorrect: true,
    });
    hintedRoundId.value = undefined;
    lastMistakeId.value = undefined;
  ...
  recordHint({
    ...
    text: round.value.explanation,
    reason: "wrong-pair-selected",
  });
...
<template>
  <GamePageShell
    gradient="linear-gradient(135deg, #f4f7ff 0%, #fff7e8 54%, #eefbf4 100%)"
    padding-top="8.75rem"
  >
    <template #hud>
      <GameHud
        ...
        <v-col cols="12" lg="11" xl="10">
          <v-card class="logic-card pa-4 pa-md-6" rounded="xl" elevation="8">
            ...
            <v-alert
              ...
              :color="hintedRoundId === round.roundId ? 'secondary' : 'primary'"
              :icon="hintedRoundId === round.roundId ? 'mdi-heart-outline' : 'mdi-link-variant'"
              rounded="xl"
              variant="tonal"
            >
              {{ helperText }}
            ...
            <v-row class="pair-layout" dense align="stretch">
              <v-col cols="12" sm="4">
                <v-sheet class="target-card pa-4" color="primary" rounded="xl">
                  ...
                  <GameWordImage
                    v-if="round.target.wordId"
                    class="target-card__visual"
                    :word-id="round.target.wordId"
                    :word="round.target.label"
                    :emoji="round.target.visual"
                  />
                  <div v-else class="target-card__visual text-white">{{ round.target.visual }}</div>
                  <div class="text-h5 text-md-h4 font-weight-bold text-white text-center mt-3">
                    {{ round.target.label }}
                  </div>
                </v-sheet>
              </v-col>
              ...
              <v-col cols="12" sm="8">
                <GameChoiceCardGrid
                  ...
                  min-height="12rem"
                  :highlight-choice="
                    (choice) => hintedRoundId === round.roundId && choice.id === round.pair.id
                  "
                  :color="
                    (choice) =>
                      hintedRoundId === round.roundId && choice.id === round.pair.id
                        ? 'primary'
                        : 'surface'
                  "
                  :cols="6"
                ...
                >
                  <template #default="{ choice }">
                    ...
                    >
                      <GameWordImage
                        v-if="choice.wordId"
                        class="choice-card__visual"
                        :word-id="choice.wordId"
                        :word="choice.label"
                        :emoji="choice.visual"
                      />
                      <div v-else class="choice-card__visual">{{ choice.visual }}</div>
                      <div class="choice-card__label text-body-1 text-md-h6 font-weight-bold mt-3">
                        {{ choice.label }}
                      </div>
                    </div>
                  </template>
