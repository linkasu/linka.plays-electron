...
export function warmLogicPairsAudio(enabled: boolean) {
  warmSoftPiano(enabled, notesToLoad);
}

export function playLogicPairsSuccessMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, successMelody);
}

export function playLogicPairsMistakeMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, mistakeMelody);
}

export function disposeLogicPairsAudio() {
  disposeSoftPiano();
}
