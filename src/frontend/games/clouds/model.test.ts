...
describe("clouds placement", () => {
  ...
  it.each([
    ...
    "keeps visual bounds separate at $width x $height across seeds and templates",
    ({ width, height }) => {
      ...
      for (const seed of [1, 7, 29, 113]) {
        for (const templateOffset of [0, 1, 2]) {
          ...
          for (const placement of layout.placements) {
            ...
            expect(placement.x - placement.visualBounds.left).toBeGreaterThanOrEqual(
              area.x - 0.001,
            );
            expect(placement.x + placement.visualBounds.right).toBeLessThanOrEqual(
              area.x + area.width + 0.001,
            );
            expect(placement.y - placement.visualBounds.top).toBeGreaterThanOrEqual(area.y - 0.001);
            expect(placement.y + placement.visualBounds.bottom).toBeLessThanOrEqual(
          ...
          for (let first = 0; first < layout.placements.length; first += 1) {
            for (let second = first + 1; second < layout.placements.length; second += 1) {
              expect(
                cloudPlacementsOverlap(layout.placements[first], layout.placements[second]),
              ).toBe(false);
            }
          }
        }
