...
import {
  generateWordCategoriesRound,
  type WordCategoriesMode,
  type WordCategory,
  type WordCategoryChoice,
} from "./model";
...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("word-categories", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  ...
  volume: 0.34,
  warmAssetIds: [
    "word-categories.intro",
    "word-categories.correct",
    "word-categories.mistake",
    "word-categories.complete",
  ],
});
...
const {
  ...
  generateRound: (roundIndex) =>
    generateWordCategoriesRound(session.settings, mode.value, roundIndex),
});
...
const targetCardTitle = computed(() =>
  round.value.mode === "item-to-category" ? "Предмет" : "Категория",
);
const targetEmoji = computed(() =>
  round.value.mode === "item-to-category"
    ? round.value.targetItem.emoji
    : round.value.targetCategory.emoji,
);
const targetLabel = computed(() =>
  round.value.mode === "item-to-category"
    ? round.value.targetItem.word
    : round.value.targetCategory.title,
);
const modeChip = computed(() =>
  round.value.mode === "item-to-category" ? "предмет → категория" : "категория → предмет",
);
...
function playIntroPrompt(delayMs = 0) {
  ...
  promptAudio.playSequence(
    ["word-categories.intro", itemAssetId(round.value.targetItem.id)],
    delayMs,
  );
}
...
async function choose(choice: WordCategoryChoice) {
  ...
  if (isCorrect) {
    ...
    recordSuccess({
      ...
      actual: choice.id,
      mode: round.value.mode,
      itemId: round.value.targetItem.id,
      categoryId: round.value.targetCategory.id,
      isCorrect: true,
    });
    const finishedAfterSuccess = session.step >= session.maxSteps;
    const successAudio = isCategoryChoice(choice)
      ? ["word-categories.correct"]
      : ["word-categories.correct", itemAssetId(choice.id)];
    isSpeaking.value = true;
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess ? [...successAudio, "word-categories.complete"] : successAudio,
      80,
      170,
    );
    isSpeaking.value = false;
          ...
          <v-card class="word-game-card pa-5 pa-md-8" rounded="xl" elevation="8">
            <div v-if="selectingMode" class="text-center">
              ...
              <p class="text-body-1 text-md-h6 text-medium-emphasis mb-6">
                Выбери один режим. Правило не изменится до конца игры.
              </p>
              <v-row justify="center">
                <v-col cols="12" sm="6">
                  <GameDwellButton
                    target-id="word-categories:mode:item-to-category"
                    :dwell-ms="session.settings.dwellMs"
                    min-height="12rem"
                    color="orange-lighten-5"
                    @select="startGame('item-to-category')"
                  >
                    <template #default>
                      <div class="d-flex flex-column align-center justify-center ga-3 pa-4">
                        ...
                        <div class="text-body-2 text-grey-darken-3">
                          Посмотри на предмет и выбери его категорию
                        </div>
                      </div>
                    </template>
                  </GameDwellButton>
                ...
                <v-col cols="12" sm="6">
                  <GameDwellButton
                    target-id="word-categories:mode:category-to-item"
                    :dwell-ms="session.settings.dwellMs"
                    min-height="12rem"
                    color="green-lighten-5"
                    @select="startGame('category-to-item')"
                  >
                    <template #default>
                      <div class="d-flex flex-column align-center justify-center ga-3 pa-4">
                        <div class="mode-emoji emoji-glyph" aria-hidden="true">🧺</div>
                        ...
                        <div class="text-body-2 text-grey-darken-3">
                          Посмотри на категорию и выбери предмет
                        </div>
                      </div>
                    </template>
                  </GameDwellButton>
            ...
            <template v-else>
              ...
              <h1 class="word-prompt text-h4 text-md-h3 font-weight-bold text-center mb-3">
                {{ round.prompt }}
              </h1>
              <p class="word-feedback text-body-1 text-center mb-6">{{ feedbackMessage }}</p>

              <v-card
                class="target-card pa-5 pa-md-6 mb-6"
              ...
              >
                ...
                <div class="d-flex flex-column flex-sm-row align-center justify-center ga-4">
                  <GameWordImage
                    v-if="round.mode === 'item-to-category'"
                    class="target-emoji"
                    :word-id="round.targetItem.id"
                    :word="round.targetItem.word"
                    :emoji="round.targetItem.emoji"
                    decorative
                  />
                  <div v-else class="target-emoji emoji-glyph" aria-hidden="true">
                    {{ targetEmoji }}
                  </div>
              ...
              <v-row justify="center">
                <v-col
                  ...
                  :key="choice.id"
                  cols="6"
                  :sm="12 / round.choices.length"
                  :md="12 / round.choices.length"
                >
                  <GameDwellButton
                  ...
                  >
                    <template #default>
                      <div class="choice-content">
                        <div
                          v-if="isCategoryChoice(choice)"
                          class="choice-emoji emoji-glyph"
                          aria-hidden="true"
                        >
                          {{ choice.emoji }}
                        </div>
