...
describe("generateWordCategoriesRound", () => {
  it("keeps the selected rule for the whole session", () => {
    ...
    expect(
      [1, 2, 3].map(
        (roundIndex) => generateWordCategoriesRound(settings, "item-to-category", roundIndex).mode,
      ),
    ).toEqual(["item-to-category", "item-to-category", "item-to-category"]);
    expect(
      [1, 2, 3].map(
        (roundIndex) => generateWordCategoriesRound(settings, "category-to-item", roundIndex).mode,
      ),
    ).toEqual(["category-to-item", "category-to-item", "category-to-item"]);
  });

  it("builds item-to-category rounds with one correct category", () => {
    const round = generateWordCategoriesRound(
  ...
  it("builds category-to-item rounds with distractors from other categories", () => {
    ...
    expect(round.choices[round.correctIndex].id).toBe(round.correctChoiceId);
    expect(
      round.choices.filter(
        (choice) => "category" in choice && choice.category === round.targetCategory.id,
      ),
    ).toHaveLength(1);
  });
  ...
  it("keeps round ids stable for telemetry", () => {
    expect(
      generateWordCategoriesRound(settingsFromPreset("standard"), "item-to-category", 8).roundId,
    ).toBe("word-categories:round:8");
  });
  ...
  it("uses complete and distinct prompts for both rules", () => {
    ...
    expect(categoryRound.prompt).toBe(
      `Какой предмет относится к категории «${categoryRound.targetCategory.title}»?`,
    );
  ...
  it.each([
    ...
    expect(generateWordCategoriesRound(settings, "item-to-category").choices).toHaveLength(
      expectedCount,
    );
    expect(generateWordCategoriesRound(settings, "category-to-item").choices).toHaveLength(
      expectedCount,
    );
  });
  ...
  it("uses injected randomness for deterministic choice order", () => {
    const settings = settingsFromPreset("standard");
    const firstOrder = generateWordCategoriesRound(
      settings,
      "item-to-category",
      1,
      () => 0,
    ...
    expect(
      generateWordCategoriesRound(settings, "item-to-category", 1, () => 0).choices.map(
        (choice) => choice.id,
      ),
    ).toEqual(firstOrder);
    expect(
      generateWordCategoriesRound(settings, "item-to-category", 1, () => 0.99).choices.map(
        (choice) => choice.id,
      ),
    ).not.toEqual(firstOrder);
  });
});
