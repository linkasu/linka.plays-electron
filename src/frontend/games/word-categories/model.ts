...
export const wordCategories: WordCategory[] = [
  {
    id: "food",
    title: "Еда",
    hint: "то, что можно есть или пить",
    emoji: "🍎",
  ...
  },
  {
    id: "transport",
    title: "Транспорт",
    hint: "то, на чём можно ехать или лететь",
    emoji: "🚗",
  ...
  },
  {
    id: "clothes",
    title: "Одежда",
    hint: "то, что надевают",
    emoji: "👕",
    color: "purple-lighten-4",
  },
];

const wordsByCategory = new Map<WordCategoryId, WordItem[]>(
  wordCategories.map((category) => [category.id, getWordsByCategory(category.id)]),
);
...
function wordsForCategory(categoryId: WordCategoryId) {
  ...
  if (words.length < 2)
    throw new Error(`WordCategoriesGame needs at least two words for ${categoryId}.`);
  return words;
}
...
function choiceCountFor(settings: SessionSettings, roundIndex: number) {
  return choiceCountByPreset(settings, roundIndex, {
    gentle: 2,
    standard: 3,
    challenge: 4,
    cap: wordCategories.length,
  });
}

function generateItemToCategoryRound(
  settings: SessionSettings,
  roundIndex: number,
  random = Math.random,
): WordCategoriesRound {
  if (categoryWords.length < wordCategories.length)
    throw new Error("WordCategoriesGame needs category words.");
  ...
  const distractors = shuffleItems(
    wordCategories.filter((category) => category.id !== correctChoiceId),
    random,
...
function generateCategoryToItemRound(
  settings: SessionSettings,
  roundIndex: number,
  random = Math.random,
): WordCategoriesRound {
...
export function generateWordCategoriesRound(
  settings: SessionSettings,
  mode: WordCategoriesMode,
  roundIndex = 1,
  random = Math.random,
