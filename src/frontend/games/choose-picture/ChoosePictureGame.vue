...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("choose-picture", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  gameId: "choose-picture",
  soundEnabled,
  warmAssetIds: ["choose-picture.correct", "choose-picture.mistake", "choose-picture.complete"],
});
  ...
  if (choice.id === round.value.target.id) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["choose-picture.correct", "choose-picture.complete"]
        : ["choose-picture.correct"],
      80,
      170,
    );
      ...
      <v-row justify="center">
        <v-col cols="12" lg="10">
          <v-card class="choose-picture-card pa-6 pa-md-8" rounded="xl" elevation="8">
            ...
            <p class="text-h6 text-medium-emphasis text-center mb-2">
              {{ choosePictureInstruction }}
            </p>
            <h1 class="text-h3 font-weight-bold text-center mb-6">{{ round.prompt }}</h1>
            <p v-if="feedbackText" class="text-h6 text-medium-emphasis text-center mb-6">
              {{ feedbackText }}
            </p>
            <GameChoiceCardGrid
