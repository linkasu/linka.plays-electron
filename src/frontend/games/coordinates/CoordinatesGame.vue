...
const hintText = computed(() => {
  if (hintedRoundId.value !== round.value.roundId)
    return "Сначала найди букву сверху, потом цифру слева.";
  ...
  if (cell.isTarget) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["coordinates.correct", "coordinates.complete"]
        : ["coordinates.correct"],
      80,
      170,
    );
      ...
      <v-row justify="center" no-gutters>
        <v-col cols="12" lg="10" xl="8">
          <v-card class="coordinates-card pa-4 pa-md-6" rounded="xl" elevation="8">
            ...
            <div
              class="coordinate-board"
              role="group"
              :aria-label="`Поле координат, цель ${round.target}`"
            >
              <div class="corner-label" aria-hidden="true"></div>
              <div
                v-for="column in coordinateColumns"
                :key="column"
                class="axis-label axis-label--column text-h4 text-md-h3 font-weight-bold"
              >
                {{ column }}
              </div>
                ...
                >
                  <template #default>
                    <div class="coordinate-cell">
                      ...
                      <div class="coordinate-cell__hint text-body-1 text-medium-emphasis">
