...
import {
  ...
  type WhatMissingItem,
  type WhatMissingPhase,
  type WhatMissingPhaseEvent,
  type WhatMissingRound,
} from "./model";
...
const {
  ...
  recordHint,
  startSession,
} = useGameSessionFor("what-missing", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  ...
  volume: 0.34,
  warmAssetIds: [
    "what-missing.observe",
    "what-missing.choose",
    "what-missing.correct",
    "what-missing.mistake",
  ],
});
...
const hintedChoiceId = computed(() =>
  hintedRoundId.value === round.value.roundId ? round.value.missingItem.id : undefined,
);
...
function isItemVisible(item: WhatMissingItem) {
  return (
    ...
    phase.value === "observe" ||
    item.id !== round.value.missingItem.id
  );
}
...
async function answer(choice: WhatMissingItem) {
  ...
  if (choice.id === round.value.missingItem.id) {
    ...
    recordSuccess({
      ...
      targetId,
      answerId: choice.id,
      expected: round.value.missingItem.label,
      actual: choice.label,
      isCorrect: true,
    });
          ...
          <v-card class="pa-4 pa-md-7" color="surface" rounded="xl" elevation="8">
            ...
            <v-row class="memory-row mb-5" dense justify="center">
              <v-col v-for="item in round.displayItems" :key="item.id" cols="4">
                <v-card
                  class="memory-slot pa-3 pa-md-5"
                  :color="isItemVisible(item) ? item.color : 'grey-lighten-4'"
                  rounded="xl"
                  elevation="0"
                >
                  ...
                  <div
                    v-else
                    class="missing-slot text-h6 text-md-h5 font-weight-bold text-medium-emphasis"
                  >
                    пропало
                  </div>
                </v-card>
              ...
              <v-row dense justify="center">
                <v-col v-for="choice in round.choices" :key="choice.id" cols="6" sm="3">
                  <GameDwellButton
                    ...
                    min-height="11.25rem"
                    :color="hintedChoiceId === choice.id ? 'deep-purple-darken-3' : 'surface'"
