...
function pickValues(settings: SessionSettings, random: () => number) {
  return shuffleItems(
    Array.from({ length: maxValueFor(settings) }, (_, index) => index + 1),
    random,
  ).slice(0, 3);
}
...
function buildCountChoices(
  ...
  const nearby = [answer - 1, answer + 1, answer - 2, answer + 2].filter(
    (value) => value >= 1 && value <= max && value !== answer,
  );
...
export function generateSimpleGraphsRound(
  settings: SessionSettings,
  roundIndex = 1,
  random = Math.random,
): SimpleGraphsRound {
  ...
  const correctBar = bars.reduce((best, bar) =>
    questionKind === "more"
      ? bar.value > best.value
        ? bar
        : best
      : bar.value < best.value
        ? bar
  ...
  return {
    ...
    helperText:
      questionKind === "more" ? "Выбери самый высокий столбик." : "Выбери самый низкий столбик.",
    questionKind,
    bars,
    choices,
    correctChoiceId,
