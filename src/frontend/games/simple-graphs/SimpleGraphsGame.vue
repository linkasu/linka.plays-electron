...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("simple-graphs", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  gameId: "simple-graphs",
  soundEnabled,
  warmAssetIds: [
    ...simpleGraphsQuestionAssetIds,
    "simple-graphs.correct",
    "simple-graphs.mistake",
    "simple-graphs.complete",
  ],
});
...
function promptAssetId() {
  if (round.value.questionKind === "count" && round.value.targetBar)
    return `simple-graphs.question.count.${round.value.targetBar.id}`;
  ...
  if (choice.choiceId === round.value.correctChoiceId) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["simple-graphs.correct", "simple-graphs.complete"]
        : ["simple-graphs.correct"],
      80,
      170,
    );
          ...
          <v-card class="simple-graphs-card pa-4 pa-md-6" rounded="xl" elevation="8">
            ...
            <v-sheet class="graph-panel pa-4 pa-md-5 mb-4" color="surface" rounded="xl" border>
              <div class="graph-bars" aria-label="Столбиковый график">
                <div
                  v-for="bar in round.bars"
                  :key="bar.id"
                  class="graph-item"
                  :class="{ 'graph-item--target': round.targetBar?.id === bar.id }"
              ...
              >
                ...
                >
                  <template #default>
                    ...
                    >
                      <GameWordImage
                        v-if="choiceBar(choice)"
                        class="choice-word-image"
                        :word-id="choiceBar(choice)!.wordId"
                        :word="choiceBar(choice)!.label"
                        :emoji="choiceBar(choice)!.emoji"
                        decorative
                      />
                      <div
                        class="choice-label font-weight-bold"
                        :class="round.questionKind === 'count' ? 'text-h2' : 'text-h5 text-md-h4'"
                      >
                        {{ choice.label }}
                      </div>
                      <div
                        v-if="round.questionKind !== 'count'"
                        class="choice-caption text-body-1 mt-2"
                      >
                        столбик
                      </div>
                    </div>
...
.graph-bar {
  ...
  background: linear-gradient(
    180deg,
    color-mix(in srgb, var(--bar-color) 72%, white),
    var(--bar-color)
  );
