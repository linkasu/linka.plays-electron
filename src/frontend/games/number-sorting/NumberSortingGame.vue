...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("number-sorting", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  gameId: "number-sorting",
  soundEnabled,
  warmAssetIds: [
    "number-sorting.prompt",
    "number-sorting.correct",
    "number-sorting.mistake",
...
const targetNumber = computed(
  () => round.value.correctOrder[selectedNumbers.value.length] ?? round.value.correctOrder[0],
);
const remainingCount = computed(() =>
  Math.max(0, round.value.correctOrder.length - selectedNumbers.value.length),
);
const directionLabel = computed(() =>
  round.value.direction === "ascending" ? "по возрастанию" : "по убыванию",
);
...
async function choose(card: NumberSortingCard) {
  ...
  if (card.value === expectedNumber) {
    ...
    if (completedSequence)
      recordSuccess({
        ...
        direction: round.value.direction,
        expected: round.value.correctOrder,
        actual: nextSelected,
        sequence: nextSelected,
        isCorrect: true,
      });
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["number-sorting.correct", "number-sorting.complete"]
        : ["number-sorting.correct"],
      80,
      170,
    );
  ...
  recordMistake({
    ...
    actual: card.value,
    sequence: [...selectedNumbers.value],
    isCorrect: false,
  });
          ...
          <v-card class="pa-4 pa-md-6" rounded="xl" elevation="8">
            ...
            <v-sheet class="pa-3 pa-md-4 mb-5" color="blue-grey-lighten-5" rounded="xl">
              <div class="d-flex flex-wrap justify-center align-center ga-2">
                <v-chip
                  v-for="number in round.correctOrder"
                  :key="number"
                  :color="selectedNumbers.includes(number) ? 'success' : undefined"
                  :variant="selectedNumbers.includes(number) ? 'flat' : 'tonal'"
                  size="large"
                >
              ...
              <div class="text-body-2 text-medium-emphasis text-center mt-2">
                Осталось выбрать: {{ remainingCount }}
              </div>
            </v-sheet>

            <v-row class="card-grid" justify="center" dense>
              <v-col
                v-for="card in round.cards"
                :key="card.id"
                cols="12"
                sm="6"
                md="4"
                class="number-card-col"
              >
                <GameDwellButton
                  :class="{
                    'number-choice--wrong': wrongNumber === card.value,
                    'number-choice--selected': selectedNumbers.includes(card.value),
                  }"
                  :target-id="cardTargetId(card)"
                  :disabled="
                    session.status !== 'running' || pendingFeedback || isSelected(card.value)
                  "
                ...
                >
                  <template #default>
                    <div class="number-card-content">
                      <v-icon
                        v-if="selectedNumbers.includes(card.value)"
                        class="number-card-icon"
