...
function shuffledBells(roundIndex: number) {
  ...
  const cards = [...bellDefinitions.slice(offset), ...bellDefinitions.slice(0, offset)].slice(
    0,
    count,
  );
...
async function selectBell(bell: BellDefinition) {
  ...
  if (bell.id !== round.value.target.id) {
    recordMistake({
      ...
      selectedId: bell.id,
      targetId: round.value.target.id,
    });
        ...
        <v-col cols="12" xl="10">
          <v-card class="bells-panel pa-4 pa-md-7" color="surface" rounded="xl" elevation="8">
            ...
            <h1 class="text-h3 text-md-h2 font-weight-bold text-center mb-3">
              Найди {{ round.target.tone }} колокольчик
            </h1>
            <p class="text-h6 text-md-h5 text-medium-emphasis text-center mb-6">
              Удержи взгляд на нужной карточке. Колокольчик качнётся и пустит волну.
            </p>

            <v-row class="bells-grid" justify="center">
              <v-col
                v-for="bell in round.cards"
                :key="`${round.roundId}-${bell.id}`"
                cols="12"
                sm="6"
                lg="3"
              >
                <GameDwellButton
                  :target-id="`bells:${round.roundId}:${bell.id}`"
                  :disabled="session.status !== 'running' || Boolean(selectedBellId) || isSpeaking"
                  :dwell-ms="session.settings.dwellMs"
                  min-height="clamp(10rem, 28dvh, 16.25rem)"
                  color="surface"
                  @select="selectBell(bell)"
                >
                  <template #default="{ active, progress }">
                    <div
                      class="bell-card-content"
                      :class="{
                        'bell-card-content--active': active,
                        'bell-card-content--selected': selectedBellId === bell.id,
                      }"
                      :style="{ '--bell-accent': bell.accent, '--bell-progress': progress }"
                    >
                      <div
                        v-if="active || selectedBellId === bell.id"
                        class="bell-wave"
                        aria-hidden="true"
                      />
                      <v-icon class="bell-icon mb-3" icon="mdi-bell-outline" />
                      <div class="bell-label text-h5 text-md-h4 font-weight-bold">
                        {{ bell.label }}
                      </div>
                      <div class="bell-tone text-body-1 mt-2">{{ bell.tone }} колокольчик</div>
                      <div class="text-body-2 mt-4 bell-hint">
                        {{
...
.bells-shell {
  background:
    radial-gradient(circle at 20% 18%, rgb(255 241 184 / 52%), transparent 34%),
    linear-gradient(135deg, #fff7dc 0%, #eef8ff 48%, #f7f0ff 100%);
...
.bell-card-content::before {
  background: radial-gradient(
    circle,
    color-mix(in srgb, var(--bell-accent) 28%, transparent),
    transparent 64%
  );
