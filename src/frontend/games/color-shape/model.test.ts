...
describe("color-shape model", () => {
  ...
  it("always offers single-trait distractors", () => {
    ...
    for (let index = 1; index <= 12; index += 1) {
      const round = generateColorShapeRound(settings, index);
      const mismatches = round.choices
        .filter((choice) => choice.id !== round.target.id)
        .map((choice) => getColorShapeMismatch(choice, round.target).join("+"));
  ...
  it("describes which traits differ from the target", () => {
    ...
    const wrongColor = round.choices.find(
      (choice) =>
        choice.shape.id === round.target.shape.id && choice.color.id !== round.target.color.id,
    );
    const wrongShape = round.choices.find(
      (choice) =>
        choice.color.id === round.target.color.id && choice.shape.id !== round.target.shape.id,
    );
    const wrongBoth = round.choices.find(
      (choice) =>
        choice.color.id !== round.target.color.id && choice.shape.id !== round.target.shape.id,
    );
