...
export const colorShapeColors: ColorShapeColor[] = [
  {
    id: "red",
    label: "красный",
    forms: { masculine: "красный", feminine: "красная" },
    hex: "#d83a3a",
    textColor: "#ffffff",
  },
  {
    id: "blue",
    label: "синий",
    forms: { masculine: "синий", feminine: "синяя" },
    hex: "#2f6fdb",
    textColor: "#ffffff",
  },
  {
    id: "green",
    label: "зелёный",
    forms: { masculine: "зелёный", feminine: "зелёная" },
    hex: "#2f9e62",
    textColor: "#ffffff",
  },
  {
    id: "yellow",
    label: "жёлтый",
    forms: { masculine: "жёлтый", feminine: "жёлтая" },
    hex: "#f8c73e",
    textColor: "#25210c",
  },
];
...
export function createColorShapeItem(
  color: ColorShapeColor,
  shape: ColorShapeShape,
): ColorShapeItem {
  return {
...
export const colorShapeItems: ColorShapeItem[] = colorShapeColors.flatMap((color) =>
  colorShapeShapes.map((shape) => createColorShapeItem(color, shape)),
);

export function getColorShapeMismatch(
  choice: ColorShapeItem,
  target: ColorShapeItem,
): ColorShapeTrait[] {
...
function pushCandidate(
  ...
  target: ColorShapeItem,
  startOffset: number,
  predicate: (item: ColorShapeItem) => boolean,
) {
...
export function generateColorShapeRound(
  ...
  pushCandidate(
    ...
    target,
    roundIndex,
    (item) => item.shape.id === target.shape.id && item.color.id !== target.color.id,
  );
  pushCandidate(
    choices,
    target,
    roundIndex + 2,
    (item) => item.color.id === target.color.id && item.shape.id !== target.shape.id,
  );
