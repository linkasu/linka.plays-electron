...
import {
  ...
  playEatOrNotEatMistakeMelody,
  warmEatOrNotEatAudio,
} from "./audio";
import {
  createEatOrNotEatRoundGenerator,
  type EatOrNotEatAnswer,
  type EatOrNotEatRound,
} from "./model";
...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("eat-or-not-eat", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  gameId: "eat-or-not-eat",
  soundEnabled: toRef(session.settings, "sound"),
  warmAssetIds: [
    "eat-or-not-eat.prompt",
    "eat-or-not-eat.correct",
    "eat-or-not-eat.mistake",
    "eat-or-not-eat.complete",
  ],
});
  ...
  if (value === round.value.correctAnswer) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["eat-or-not-eat.correct", "eat-or-not-eat.complete"]
        : ["eat-or-not-eat.correct"],
      80,
      170,
    );
          ...
          <v-card class="eat-card pa-6 pa-md-8" rounded="xl" elevation="8">
            ...
            <v-row>
              <v-col v-for="option in answers" :key="option.id" cols="12" sm="6">
                <GameDwellButton
                  :target-id="answerTargetId(option.id)"
