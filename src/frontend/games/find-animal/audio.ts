...
export function warmFindAnimalAudio(enabled: boolean) {
  warmSoftPiano(enabled, notesToLoad);
}

export function playFindAnimalSuccessMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, successMelody);
}

export function playFindAnimalMistakeMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, mistakeMelody);
}

export function disposeFindAnimalAudio() {
  disposeSoftPiano();
}
