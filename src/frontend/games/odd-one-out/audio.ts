...
export function warmOddOneOutAudio(enabled: boolean) {
  warmSoftPiano(enabled, notesToLoad);
}

export function playOddOneOutSuccessMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, successMelody);
}

export function playOddOneOutMistakeMelody(enabled: boolean) {
  return playSoftPianoMelody(enabled, mistakeMelody);
}

export function disposeOddOneOutAudio() {
  disposeSoftPiano();
}
