...
export function boardShapeFor(settings: SessionSettings): BoardShape {
  if (settings.preset === "gentle" || settings.preset === "challenge")
    return boardShapes[settings.preset];
...
export function createInitialCellStates(board: MinesweeperSafeBoard): MinesweeperSafeCellState[] {
  return board.cells.map((cell) =>
    board.initialRevealed.includes(cell.index) ? "revealed" : "hidden",
  );
}

export function findSuggestedSafeIndex(
  cells: MinesweeperSafeCell[],
  states: MinesweeperSafeCellState[],
) {
  const revealed = new Set(
    cells.filter((cell) => states[cell.index] === "revealed").map((cell) => cell.index),
  );
  ...
  return hiddenSafe
    .map((cell) => ({
      cell,
      revealedNeighbors: adjacentIndexes(cell.index, Math.trunc(Math.sqrt(cells.length))).filter(
        (index) => revealed.has(index),
      ).length,
    }))
    .sort(
      (a, b) =>
        b.revealedNeighbors - a.revealedNeighbors ||
        a.cell.adjacentMines - b.cell.adjacentMines ||
        a.cell.index - b.cell.index,
    )[0]?.cell.index;
}

export function minesweeperSafeChoiceOutcome(
  cell: MinesweeperSafeCell,
  state: MinesweeperSafeCellState,
): MinesweeperSafeChoiceOutcome {
  if (state !== "hidden") return "ignored";
  return cell.mine ? "mine" : "safe";
}

export function areAllMinesFlagged(
  cells: MinesweeperSafeCell[],
  states: MinesweeperSafeCellState[],
) {
  return cells.every((cell) =>
    cell.mine ? states[cell.index] === "flagged" : states[cell.index] !== "flagged",
  );
}

export function generateMinesweeperSafeBoard(
  settings: SessionSettings,
  roundIndex = 1,
  random = Math.random,
): MinesweeperSafeBoard {
  const shape = boardShapeFor(settings);
  const totalCells = shape.size * shape.size;
  const center = Math.floor(totalCells / 2);
  const reserved = new Set(
    [center, center - 1, center + 1, center - shape.size, center + shape.size].filter(
      (index) => index >= 0 && index < totalCells,
    ),
  );
  const mineIndexes = new Set(
    shuffleIndexes(totalCells, random)
  ...
  const cells = Array.from({ length: totalCells }, (_, index) => {
    ...
    const adjacentMines = adjacentIndexes(index, shape.size).filter((neighbor) =>
  ...
  const initialRevealed = cells
    ...
    .sort((a, b) => {
      const centerDistanceA =
        Math.abs(a.row - Math.floor(shape.size / 2)) + Math.abs(a.col - Math.floor(shape.size / 2));
      const centerDistanceB =
        Math.abs(b.row - Math.floor(shape.size / 2)) + Math.abs(b.col - Math.floor(shape.size / 2));
      return (
        b.adjacentMines - a.adjacentMines || centerDistanceA - centerDistanceB || a.index - b.index
      );
    })
