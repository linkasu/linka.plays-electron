...
describe("generateWhereObjectRound", () => {
  it("makes the visible cue match exactly one visual mini-scene", () => {
    ...
    for (let index = 1; index <= whereObjectItems.length; index += 1) {
      ...
      const cueMatches = round.choices.filter(
        (choice) => round.prompt === `Покажи: ${choice.scenePhrase}.`,
      );
      ...
      expect(round.choices.every((choice) => choice.targetObject === round.targetObject)).toBe(
        true,
      );
  ...
  it("uses on, under, in and beside in every round", () => {
    ...
    expect(new Set(round.choices.map((choice) => choice.preposition.id))).toEqual(
      new Set(["on", "under", "in", "beside"]),
    );
  });
  ...
  it("deals a balanced shuffled relation/object deck without repeats", () => {
    ...
    expect(
      new Set(rounds.map((round) => `${round.targetObject.id}:${round.targetPreposition.id}`)),
    ).toHaveLength(deckSize);
    ...
    for (let index = 0; index < rounds.length; index += whereObjectItems.length) {
      ...
      expect(new Set(batch.map((round) => round.targetObject.id))).toHaveLength(
        whereObjectItems.length,
      );
      for (const preposition of whereObjectPrepositions) {
        expect(batch.filter((round) => round.targetPreposition.id === preposition.id)).toHaveLength(
          2,
        );
      }
    }
  ...
  it("uses randomness instead of deriving the deck and card positions from the round index", () => {
    ...
    const signature = (generateRound: ReturnType<typeof createWhereObjectRoundGenerator>) =>
      Array.from({ length: 8 }, (_, index) => {
        const round = generateRound(index + 1);
        const correctPosition = round.choices.findIndex((choice) =>
          isWhereObjectCorrect(round, choice),
        );
  ...
  it("builds grammatical scene phrases", () => {
    ...
    const rounds = Array.from({ length: whereObjectItems.length }, (_, index) =>
      generateRound(index + 1),
    );

    for (const round of rounds) {
      expect(round.scenePhrase).toBe(
        `${round.targetObject.word} ${phraseFor(round.targetPlace, round.targetPreposition)}`,
      );
