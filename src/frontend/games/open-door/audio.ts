...
const doorPattern = [43, 50, 55, 62, 59, 55, 47, 52, 59, 64, 62, 52, 50, 55, 62, 67, 64, 59, 47, 55, 62, 71, 67, 62] as const;
const cueNotes = [55, 62, 67, 76] as const;
...
async function ensurePiano(resumeAudio: boolean) {
  ...
  loading = (async () => {
    try {
      ...
      try {
        nextPiano.output.addEffect("open-door-soft-room", Reverb(context), 0.26);
      } catch {
        // Reverb is optional: opening the door must keep working with dry piano or silence.
      }
...
function scheduleLoop(instrument: SoftPiano) {
  ...
  doorPattern.forEach((note, index) => {
    instrument.start({
      note,
  ...
  scheduledUntil = startAt + doorPattern.length * 0.4;
}
