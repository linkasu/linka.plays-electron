...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("lines-angles", {
  maxSteps: 8,
...
const linesAnglesTaskAssetIds = [
  "lines-angles.task.straight",
  "lines-angles.task.curved",
  "lines-angles.task.angle",
  "lines-angles.task.no-angle",
  "lines-angles.task.vertical",
  "lines-angles.task.horizontal",
];
const promptAudio = useGamePromptAudio({
  gameId: "lines-angles",
  soundEnabled,
  warmAssetIds: [
    ...linesAnglesTaskAssetIds,
    "lines-angles.correct",
    "lines-angles.mistake",
    "lines-angles.complete",
  ],
});
...
const hintText = computed(() => {
  if (hintedRoundId.value !== round.value.roundId)
    return "Посмотри на линии и выбери подходящую карточку.";
  ...
  if (choice.id === round.value.target.id) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["lines-angles.correct", "lines-angles.complete"]
        : ["lines-angles.correct"],
      80,
      170,
    );
          ...
          <v-card class="lines-angles-card pa-4 pa-md-7" rounded="xl" elevation="8">
            ...
            <v-row class="choice-grid" justify="center" dense>
              <v-col
                ...
                :key="choice.id"
                class="geometry-choice-col"
                cols="12"
                sm="6"
                :md="round.choices.length <= 3 ? 4 : round.choices.length === 4 ? 3 : 4"
              >
                <GameDwellButton
                ...
                >
                  <template #default>
                    <div
                      :class="['geometry-card', { 'geometry-card--mistake': isMistake(choice) }]"
                    >
                      <svg
                        class="geometry-svg"
                        viewBox="0 0 160 120"
                      ...
                      >
                        <line
                          v-if="choice.id === 'straight-vertical'"
                          x1="80"
                          y1="16"
                          x2="80"
                          y2="104"
                        />
                        <line
                          v-else-if="choice.id === 'straight-horizontal'"
                          x1="24"
                          y1="60"
                          x2="136"
                          y2="60"
                        />
                        <line
                          v-else-if="choice.id === 'straight-diagonal'"
                          x1="34"
                          y1="94"
                          x2="126"
                          y2="26"
                        />
                        <path
                          v-else-if="choice.id === 'curved-vertical'"
                          d="M88 14 C44 34 116 58 72 106"
                        />
                        <path
                          v-else-if="choice.id === 'curved-horizontal'"
                          d="M22 68 C48 20 106 104 138 52"
                        />
                        <path
                          v-else-if="choice.id === 'wave-curve'"
                          d="M18 62 C42 28 62 96 84 62 S124 28 144 62"
                        />
                        <path
                          v-else-if="choice.id === 'round-arc'"
                          d="M42 90 C50 28 112 28 120 90"
                        />
