...
const {
  ...
  recordMistake,
  startSession,
} = useGameSessionFor("color-pattern", {
  maxSteps: 8,
...
async function choose(choice: ColorPatternColor) {
  ...
  if (choice.id === round.value.answer.id) {
    ...
    recordSuccess({
      ...
      actual: choice.label,
      patternKind: round.value.patternKind,
      isCorrect: true,
    });
    ...
    const playback = await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["color-pattern.correct", "color-pattern.complete"]
        ...
        <v-col cols="12" lg="10" xl="9">
          <v-card class="pa-4 pa-md-5" rounded="xl" elevation="8">
            ...
            <div
              ...
              role="group"
              aria-label="Цветовой ряд с пустой последней карточкой"
            >
              <v-card
                v-for="(color, index) in round.sequence"
              ...
              >
                <span class="text-h6 text-md-h5 font-weight-bold color-label">{{
                  color.label
                }}</span>
              </v-card>
              <v-card
                ...
                >
                  <template #default>
                    <div class="choice-card" :style="colorStyle(choice)">
                      <span class="text-h4 text-md-h3 font-weight-bold color-label">{{
                        choice.label
                      }}</span>
                    </div>
                  </template>
