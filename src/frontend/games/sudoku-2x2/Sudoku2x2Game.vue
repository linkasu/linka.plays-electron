...
import {
  generateSudoku2x2Round,
  sudoku2x2Choices,
  type Sudoku2x2Choice,
  type Sudoku2x2Value,
} from "./model";
...
const choiceMinHeight = computed(() =>
  round.value.size >= 4 ? "clamp(5.5rem, 13vh, 8.5rem)" : "clamp(7rem, 18vh, 11rem)",
);

const feedbackText = computed(() => {
  if (mistakesInRound.value === 0)
    return `Посмотри на строку и столбик. В каждом должны быть числа от 1 до ${round.value.size}.`;
...
async function answer(choice: Sudoku2x2Choice) {
  ...
  if (choice.id === round.value.correctChoice.id) {
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess ? ["sudoku-2x2.correct", "sudoku-2x2.complete"] : ["sudoku-2x2.correct"],
      80,
      170,
    );
        ...
        <v-col cols="12" lg="10" xl="8">
          <v-card class="sudoku-card pa-4 pa-md-7" rounded="xl" elevation="8">
            ...
            <v-alert
              ...
              :color="mistakesInRound > 0 ? 'secondary' : 'primary'"
              :icon="mistakesInRound > 0 ? 'mdi-heart-outline' : 'mdi-check'"
              rounded="xl"
              variant="tonal"
            >
              {{ feedbackText }}
            ...
            <div class="sudoku-layout">
              <div
                class="sudoku-board"
                :style="boardStyle"
                :aria-label="`Поле судоку ${round.size} на ${round.size}`"
              >
                <div
                  v-for="cell in round.board"
                  :key="cell.id"
                  :class="[
                    'sudoku-cell',
                    cell.hidden ? 'sudoku-cell--missing' : cellTone(cell.value),
                  ]"
                >
                  ...
                  <template v-else>
                    ...
                    <span class="sudoku-cell__caption text-body-2 font-weight-bold">{{
                      sudoku2x2Choices.find((choice) => choice.value === cell.value)?.colorName
                    }}</span>
                  </template>
                </div>
              </div>
              ...
              <div class="choice-panel">
                <div class="text-h5 text-md-h4 font-weight-bold text-center mb-3">
                  Выбери карточку
                </div>
                  ...
                  >
                    <template #default>
                      <div
                        :class="[
                          'choice-card',
                          `choice-card--${choice.tone}`,
                          { 'choice-card--mistake': choice.id === lastMistakeId },
                        ]"
                      >
                        <span class="choice-card__number">{{ choice.label }}</span>
                        <span class="choice-card__caption text-body-1 text-md-h6 font-weight-bold"
                          >{{ choice.colorName }} карточка</span
                        >
                      </div>
                    </template>
                  </GameDwellButton>
...
.sudoku-board {
  ...
  --sudoku-cell-size: clamp(
    3.2rem,
    min(var(--sudoku-cell-vh), calc(40vw / var(--sudoku-size))),
    8rem
  );
}
...
@media (max-width: 56rem) {
  ...
  .sudoku-board {
    --sudoku-cell-size: clamp(
      3.2rem,
      min(var(--sudoku-cell-vh), calc(78vw / var(--sudoku-size))),
      7rem
    );
  }
}
