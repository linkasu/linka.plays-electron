...
export const solfegeMaxSteps = solfegeGuidedStages.reduce(
  (total, stage) => total + stage.sequence.length,
  0,
);

export function solfegeGuidedStageStart(stageIndex: number) {
  return solfegeGuidedStages
    .slice(0, stageIndex)
    .reduce((total, stage) => total + stage.sequence.length, 0);
}
...
export function solfegeGuidedProgress(completedSteps: number) {
  ...
  for (let stageIndex = 0; stageIndex < solfegeGuidedStages.length; stageIndex += 1) {
    ...
    if (noteIndex < stage.sequence.length)
      return { stageIndex, stage, noteIndex, expectedNote: stage.sequence[noteIndex] };
  }
...
export function solfegeSelectionOutcome(
  mode: SolfegeMode,
  noteId: string,
