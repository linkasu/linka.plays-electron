...
async function ensurePiano(resumeAudio: boolean) {
  ...
  loading = (async () => {
    try {
      ...
      try {
        nextPiano.output.addEffect("starry-sky-room", Reverb(context), 0.22);
      } catch {
        // Reverb is optional: constellation drawing must continue with silence or dry piano.
      }
