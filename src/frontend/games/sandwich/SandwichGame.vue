...
const feedbackMessage = ref(
  "Собираем бутерброд по рецепту: хлеб, масло, начинка, овощи и верхний хлеб.",
);
...
const ingredientAudio = useGamePromptAudio({
  ...
  soundEnabled: toRef(session.settings, "sound"),
  warmAssetIds: [
    "word-categories.item.bread",
    "word-categories.item.cheese",
    "word-categories.item.tomato",
  ],
});
let feedbackTimer = 0;
let recipeTimer = 0;
...
const completedSandwiches = computed(() =>
  Math.floor(session.step / currentRecipe.value.steps.length),
);
const plateLayers = computed(() =>
  plateSteps.value.map((step, index) => ({ ...step, layerKey: `${step.id}:${index}` })),
);
const currentPrompt = computed(
  () => currentStep.value?.instruction ?? "Бутерброды готовы. Спасибо за сборку.",
);
...
async function chooseIngredient(choice: SandwichChoice) {
  ...
  if (choice.id === expectedStep.choice.id) {
    ...
    recordSuccess({
      roundId: expectedStep.roundId,
      targetId,
      expectedTargetId,
      expected: expectedStep.choice.id,
      actual: choice.id,
      recipeId: expectedStep.recipeId,
      isCorrect: true,
    });
    ...
    if (session.status === "running" && isSandwichRecipeCompleteStep(expectedStep)) {
      await promptAudio.playSequenceAndWait(
        ["sandwich.correct", "sandwich.recipe-complete"],
        80,
        170,
      );
      isFeedbackPlaying.value = false;
...
<template>
  <GamePageShell
    class="sandwich-page"
    gradient="linear-gradient(135deg, #fff8e1 0%, #eef8ee 54%, #fff3e0 100%)"
    padding-top="4rem"
  >
    <template #hud>
      <GameHud
      ...
      <v-row justify="center" no-gutters>
        <v-col cols="12" xl="10">
          <v-card class="sandwich-card pa-3 pa-md-5" rounded="xl" elevation="8">
            ...
            <p class="intro text-body-1 text-medium-emphasis text-center mb-3">
              Выбирай ингредиенты по рецепту. Готовый бутерброд немного побудет на тарелке, потом
              начнём следующий.
            </p>
            ...
            <div class="recipe-strip mb-3" :aria-label="`Рецепт: ${currentRecipe.helper}`">
              <div
                ...
                :class="[
                  'recipe-step',
                  {
                    'recipe-step--done': isRecipeResting || index < currentStep.stepIndex,
                    'recipe-step--current': !isRecipeResting && index === currentStep.stepIndex,
                  },
                ]"
              >
                <div class="recipe-step-visual">
                  ...
                  <v-icon
                    v-if="ingredient.roleIcon"
                    class="ingredient-role"
                    :icon="ingredient.roleIcon"
                    size="small"
                  />
                </div>
                <div class="recipe-step-label text-caption font-weight-bold">
                  {{ ingredient.label }}
                </div>
              </div>
            </div>
            ...
            <v-card
              class="prompt-card pa-3 pa-md-4 mb-3"
              ...
              <div class="d-flex flex-wrap align-center ga-3">
                ...
                >
                  <GameWordImage
                    ...
                    decorative
                  />
                  <span v-else class="emoji-glyph" aria-hidden="true">{{
                    currentStep.choice.emoji
                  }}</span>
                  <v-icon
                    v-if="currentStep.choice.roleIcon"
                    class="ingredient-role"
                    :icon="currentStep.choice.roleIcon"
                    size="small"
                  />
                </div>
                <div>
                  <div class="text-caption text-medium-emphasis">
                    {{ currentRecipe.title }} · шаг {{ currentStep.stepIndex + 1 }} из
                    {{ currentRecipe.steps.length }}
                  </div>
                  <div class="text-h6 text-md-h5 font-weight-bold">
                    {{ isRecipeResting ? `${currentRecipe.title} готов` : currentPrompt }}
                  </div>
            ...
            <div class="play-area">
              ...
              >
                <div class="plate-stage" aria-label="Собранный бутерброд">
                  ...
                  <div class="plate">
                    <div
                      v-if="plateLayers.length === 0"
                      class="empty-plate text-body-1 text-medium-emphasis"
                    >
                      Тарелка ждёт нижний хлеб
                    </div>
                    <div
                      v-for="layer in plateLayers"
                      :key="layer.layerKey"
                      :class="['sandwich-layer', `sandwich-layer--${layer.choice.kind}`]"
                    ...
                    >
                      <GameWordImage
                        ...
                        decorative
                      />
                      <span v-else class="emoji-glyph mr-2" aria-hidden="true">{{
                        layer.choice.emoji
                      }}</span>
                ...
                <div class="d-flex flex-wrap justify-center ga-2 mt-3">
                  ...
                  <v-chip color="primary" variant="tonal"
                    >Шаг {{ Math.min(session.step + 1, session.maxSteps) }} из
                    {{ session.maxSteps }}</v-chip
                  >
                </div>
              </v-card>
              ...
              <div class="ingredient-grid" aria-label="Ингредиенты для бутерброда">
                <GameDwellButton
                  ...
                  @select="chooseIngredient(choice)"
                >
                  <template #default>
                    <div class="ingredient-content">
                      <div class="choice-visual" :style="{ backgroundColor: choice.color }">
                        ...
                        <v-icon
                          v-if="choice.roleIcon"
                          class="ingredient-role"
                          :icon="choice.roleIcon"
                          size="small"
                        />
                      </div>
...
.plate {
  ...
  border-radius: 50%;
  box-shadow:
    inset 0 0 0 0.75rem rgb(255 255 255 / 74%),
    0 1.125rem 2.125rem rgb(121 85 72 / 18%);
  display: flex;
