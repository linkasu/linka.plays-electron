...
const {
  ...
  startSession,
  finishSession,
} = useGameSessionFor("greater-less", {
  maxSteps: 8,
...
const promptAudio = useGamePromptAudio({
  ...
  warmAssetIds: [
    ...
    "greater-less.mistake",
    "greater-less.complete",
  ],
});
...
async function choose(side: GreaterLessSide) {
  ...
  const expectedCount =
    round.value.correctSide === "left" ? round.value.left.count : round.value.right.count;
  ...
  if (side === round.value.correctSide) {
    ...
    recordSuccess({
      ...
      expectedSide: round.value.correctSide,
      selectedSide: side,
    ...
    await promptAudio.playSequenceAndWait(
      finishedAfterSuccess
        ? ["greater-less.correct", "greater-less.complete"]
        : ["greater-less.correct"],
      80,
      170,
    );
        ...
        <v-col cols="12" lg="11">
          <v-card class="pa-4 pa-md-6" rounded="xl" elevation="8">
            ...
            <v-row class="choice-row" dense>
              <v-col
                v-for="group in [round.left, round.right]"
                :key="group.side"
                cols="12"
                sm="6"
                md="6"
              >
                <GameDwellButton
                  :target-id="sideTargetId(group.side)"
                  :disabled="session.status !== 'running' || isSpeaking"
                  :dwell-ms="session.settings.dwellMs"
                  :min-height="250"
                  color="surface"
                  @select="choose(group.side)"
                >
                  <template #default>
                    <div class="group-label text-overline mb-3">
                      {{ group.side === "left" ? "Слева" : "Справа" }}
                    </div>
                    <div class="group-items" aria-hidden="true">
                      <GameWordImage
                        v-for="(_item, index) in group.items"
                        :key="index"
                        class="group-emoji"
                        :word-id="group.itemId"
                        :word="group.itemName"
                        :emoji="group.emoji"
                        decorative
                      />
                    ...
                    <div class="sr-only">
                      {{ group.side === "left" ? "Левая" : "Правая" }} группа
                    </div>
                  </template>
                </GameDwellButton>
              </v-col>
