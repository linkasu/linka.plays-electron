...
import {
  clampTargetCenterPercent,
  createSafePlacementArea,
  randomSeparatedTargetCenterPercent,
...
describe("placement", () => {
  ...
  it("clamps target centers so the whole target stays visible", () => {
    const point = clampTargetCenterPercent(
      { x: 0, y: 0 },
      {
        viewportWidth: 1000,
        viewportHeight: 700,
  ...
  it("keeps random placement away from multiple active points when possible", () => {
    ...
    const distanceToAvoid = avoid.map((other) =>
      Math.hypot(point.x * 10 - other.x * 10, point.y * 7 - other.y * 7),
    );
