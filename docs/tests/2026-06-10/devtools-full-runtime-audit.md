# Full Electron runtime audit — 2026-06-10

Проверка выполнена через реальный Electron CDP target `LINKa plays` на `127.0.0.1:9222`. Проверялись все игры из `src/frontend/data/games.ts` на viewport `1024x600` и `800x600`.

JSON с полными метриками: `devtools-full-runtime-audit.json`.

Разбор ложных и подтвержденных флагов: `devtools-runtime-triage.md`.
...
| Игр с runtime-флагами | 93 |
| Route mismatch | 0 |
| Blank-like DOM flags | 16 |
| Console/runtime errors | 0 |
| Horizontal overflow | 2 |
| HUD/target overlaps | 3 |
| Нет видимых targets при наличии targets | 43 |
| Видимые targets меньше 88px | 4 |
...
| [ducks](./ducks.md) | Утки | gaze-basics | blank | 1024x600: targets 0/0, min 0, y 1, cardsOut 0<br>800x600: targets 0/0, min 0, y 1, cardsOut 0 |
| [firefly-meadow](./firefly-meadow.md) | Светлячковая поляна | gaze-basics | blank | 1024x600: targets 0/0, min 0, y 1, cardsOut 0<br>800x600: targets 0/0, min 0, y 1, cardsOut 0 |
| [fishes](./fishes.md) | Рыбки | gaze-basics | blank | 1024x600: targets 0/0, min 0, y 1, cardsOut 0<br>800x600: targets 0/0, min 0, y 1, cardsOut 0 |
| [flowers](./flowers.md) | Цветы | gaze-basics | blank | 1024x600: targets 0/0, min 0, y 1, cardsOut 0<br>800x600: targets 0/0, min 0, y 1, cardsOut 0 |
...
| [frog](./frog.md) | Жаба | gaze-basics | blank | 1024x600: targets 0/0, min 0, y 1, cardsOut 0<br>800x600: targets 0/0, min 0, y 1, cardsOut 0 |
...
| [logic-pairs](./logic-pairs.md) | Логические пары | numeracy | many-cards-below-fold | 1024x600: targets 4/4, min 210, y 1.4, cardsOut 3<br>800x600: targets 2/4, min 210, y 1.71, cardsOut 5 |
...
| [quiet-bubbles](./quiet-bubbles.md) | Тихие пузыри | gaze-basics | blank | 1024x600: targets 0/0, min 0, y 1, cardsOut 0<br>800x600: targets 0/0, min 0, y 1, cardsOut 0 |
...
| [table-tennis](./table-tennis.md) | Теннис | continuous-control | blank | 1024x600: targets 0/0, min 0, y 1, cardsOut 0<br>800x600: targets 0/0, min 0, y 1, cardsOut 0 |
| [tic-tac-toe](./tic-tac-toe.md) | Крестики-нолики | strategy | small-visible-target | 1024x600: targets 9/12, min 72, y 1.52, cardsOut 1<br>800x600: targets 9/12, min 72, y 1.51, cardsOut 1 |
...
| [gaze-maze](./gaze-maze.md) | Лабиринт взгляда-указателя | visual-search | horizontal-overflow | 800x600: targets 5/7, min 105, y 1.66, cardsOut 1 |
| [hide-and-seek](./hide-and-seek.md) | Прятки | visual-search | hud-target-overlap | 800x600: targets 5/5, min 150, y 1, cardsOut 0 |
...
| [musical-path](./musical-path.md) | Музыкальная дорожка | sequencing | hud-target-overlap | 800x600: targets 8/8, min 142, y 1, cardsOut 0 |
...
- `very-tall-page` не всегда баг: некоторые карточные экраны допускают прокрутку. Для gaze-first игр это флаг ручной проверки, потому основные цели могут быть ниже первого экрана.
- `many-cards-below-fold` указывает на риск плохой первой раскладки, особенно на `800x600`.
- `blank-like DOM flags` после triage оказались canvas-only false positives: у `firefly-meadow`, `butterfly`, `flowers`, `quiet-bubbles`, `ducks`, `fishes`, `frog`, `table-tennis` canvas видим и рисует кадры.
- Отсутствие route mismatch и console/runtime errors подтверждено для всех 169 игр.
- Horizontal overflow, HUD/target overlaps и small targets остаются подтвержденными runtime-дефектами и вынесены в `devtools-runtime-triage.md`.
