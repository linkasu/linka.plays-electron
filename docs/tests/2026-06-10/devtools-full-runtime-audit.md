...
- `blank-like DOM flags` после triage оказались canvas-only false positives: у `firefly-meadow`, `butterfly`, `flowers`, `bubble-pop`, `ducks`, `fishes`, `frog`, `table-tennis` canvas видим и рисует кадры.
