...
Пошаговый тетрис без таймера падения: ребёнок двигает фигуру кнопками, поворачивает её и ставит вниз в темпе.

Самостоятельная формулировка для меню: Поставь фигуру на поле и продолжай последовательность ходов.

## Игровой цикл

```text
показ фигуры и поля -> выбор сдвига, поворота или постановки -> фигура фиксируется, поле обновляется, top-out завершает доску -> итог
```
