...
Ребёнок подбирает пару по смыслу, форме или числу: ключ к замку, форма к форме, цифра к точкам. Игра тренирует классификацию и связь между объектами.

Самостоятельная формулировка для меню: найди подходящую пару.
...
|---|---|
| Route в router | есть |
| Vue-компонент | `LogicPairsGame.vue` |
| Model | `model.ts` |
| Model test | `model.test.ts` |
| Audio module | `audio.ts` |
| Runtime audit doc | `docs/tests/2026-06-10/logic-pairs.md` |
...
- TTS проговаривает тип пары, успех и ошибку с повтором задания.
