...
Ребёнок ищет частично скрытого персонажа в фигура-фон сцене. Игра тренирует зрительный поиск, выделение объекта и выбор.

Самостоятельная формулировка для меню: найди, кто спрятался.
...
|---|---|
| Route в router | есть |
| Vue-компонент | `WhoHidingGame.vue` |
| Model | нет отдельной модели |
| Model test | нет |
| Audio module | нет |
| Runtime audit doc | `docs/tests/2026-06-10/who-hiding.md` |
...
- TTS подробно проговаривает старт, цель раунда, успех, ошибку без наказания и конкретную подсказку по укрытию.
