...
Ребёнок смотрит на крупного воздушного змея, и взгляд удерживает его в воздухе. Игра поддерживает фиксацию и слежение взглядом.

Самостоятельная формулировка для меню: подними змея взглядом.
...
|---|---|
| Route в router | есть |
| Vue-компонент | `KiteGame.vue` |
| Model | нет отдельной модели |
| Model test | нет |
| Audio module | `audio.ts` |
| Runtime audit doc | `docs/tests/2026-06-10/kite.md` |
...
- Музыка включается только при разрешённом звуке и активной сессии; громкость растёт вместе с высотой змея.
