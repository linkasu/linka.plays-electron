...
Спокойная версия сапёра: ребёнок открывает клетки по числовым подсказкам, учится оценивать риск и может запросить мягкую подсказку.
...
```text
показ поля с начальными числами -> выбор закрытой клетки или ручной подсказки -> безопасная клетка открывается, опасная мягко останавливает раунд -> следующая доска или итог
```
