...
Пятнашки 3×3 в режиме: ребёнок ищет плитку-соседа пустой клетки и собирает последовательность ходов без раскрытия готового ответа.

Самостоятельная формулировка для меню: Передвигай соседние плитки в пустое место и собирай картинку шаг за шагом.

## Игровой цикл

```text
показ поля -> выбор плитки -> соседняя плитка переходит в пустое место, повтор правила без подсветки возможных ходов -> итог
```
