...
Пятнашки 3×3 в мягком режиме: ребёнок ищет плитку-соседа пустой клетки и собирает последовательность ходов без раскрытия готового ответа.
...
```text
показ поля -> выбор плитки -> соседняя плитка переходит в пустое место, ошибка повторяет правило без подсветки возможных ходов -> итог
```
