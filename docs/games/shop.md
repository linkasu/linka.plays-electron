...
Выбрать список покупок в пределах кошелька или собрать оплату монетами. В раунде со списком покупок остаток монет разрешён: если есть 10 монет, можно купить товары на меньшую сумму, например яблоко и хлеб. Ошибка оставляет текущий раунд и не показывает готовое решение.
...
```text
озвучивание задания -> выбор товаров в корзину или монет для оплаты -> проверка -> следующий пример
```
