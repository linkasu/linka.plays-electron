...
Ребёнок собирает поезд из пяти вагонов по номерному правилу от 1 до 5. Вагончики каждый раз лежат в случайном порядке, поэтому нужно смотреть на номера, а не на позицию.

Самостоятельная формулировка для меню: Прицепи вагоны.

## Игровой цикл

```text
случайная раскладка вагонов -> выбор вагона по правилу 1→5 -> вагон прицеплен -> следующий выбор или отправление поезда
```
...
- TTS проговаривает только общее правило: собрать поезд по номерам от 1 до 5.
- Верный выбор, повторная попытка и завершение получают короткую нейтральную TTS-фразу и мягкий piano-feedback.
- Пока звучит TTS/feedback, выбор заблокирован.
- Повторная попытка не подсвечивает и не называет следующий правильный вагон.
...
- Вагончики должны раскладываться в случайном порядке при старте/рестарте.
- UI не должен раскрывать конкретный следующий вагон отдельной подсказкой.
