...
Найти заданную форму среди крупных карточек. Задание каждого раунда озвучивается через TTS. Ошибка оставляет тот же раунд и не называет правильную карточку сверх текущего задания.
...
```text
озвучивание формы -> показ крупных карточек -> выбор ответа крупной целью -> проверка -> следующий пример
```
...
- Минимальная short-side эвристика targets: 0.267.
