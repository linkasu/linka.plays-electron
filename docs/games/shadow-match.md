...
- Минимальная short-side эвристика targets: 0.231.
...
|---|---|
| Route в router | есть |
| Vue-компонент | `ShadowMatchGame.vue` |
| Model | `model.ts` |
| Model test | `model.test.ts` |
| Audio module | `audio.ts` |
| Runtime audit doc | `docs/tests/2026-06-10/shadow-match.md` |
...
- TTS объясняет задачу без подсказки правильного предмета, мягкий успех и спокойную ошибку с повтором задания.
