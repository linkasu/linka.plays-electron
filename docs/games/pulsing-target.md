...
| Default dwell | 900 ms |

## Назначение

Ребёнок удерживает взгляд или указатель в большой пульсирующей зоне. Цель движется мягко, замедляется рядом с input и сохраняет прогресс при возвращении.

Самостоятельная формулировка для меню: удержи взгляд на цели.
...
- Базовый dwell из registry: 900 ms.
...
|---|---|
| Route в router | есть |
| Vue-компонент | `PulsingTargetGame.vue` |
| Model | `model.ts` |
| Model test | `model.test.ts` |
| Audio module | нет |
| Runtime audit doc | `docs/tests/2026-06-10/pulsing-target.md` |
