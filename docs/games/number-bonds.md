...
Найти недостающую часть состава числа. Вопрос каждого раунда озвучивается через TTS. Визуальная схема не показывает количество скрытых точек, а повторная попытка не раскрывает ответ.
...
```text
озвучивание вопроса -> показ числовой задачи -> выбор ответа крупной целью -> проверка -> следующий пример
```
