...
AAC-игра без неправильных ответов: ребёнок выбирает `хочу` или `не хочу` для предмета или занятия, а система уважительно озвучивает выбор.

Самостоятельная формулировка для меню: скажи, хочешь или не хочешь.
...
- Минимальная short-side эвристика targets: 0.293.
...
|---|---|
| Route в router | есть |
| Vue-компонент | `WantDontWantGame.vue` |
| Model | `model.ts` |
| Model test | `model.test.ts` |
| Audio module | TTS через `useGamePromptAudio` |
| Runtime audit doc | `docs/tests/2026-06-10/want-dont-want.md` |
