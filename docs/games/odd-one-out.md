...
Ребёнок выбирает предмет, который отличается от общей группы, а подсказка объясняет категорию. Игра тренирует классификацию и visual reasoning.

Самостоятельная формулировка для меню: найди лишний предмет.
...
- TTS объясняет задачу без подсказки правильной карточки, успех и ошибку с повтором задания.
