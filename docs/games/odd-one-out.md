...
Ребёнок выбирает предмет, который отличается от общей группы, а подсказка мягко объясняет категорию. Игра тренирует классификацию и visual reasoning.
...
- TTS объясняет задачу без подсказки правильной карточки, мягкий успех и спокойную ошибку с повтором задания.
