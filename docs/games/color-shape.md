...
Найти карточку, где одновременно совпадают цвет и форма. Задание каждого раунда озвучивается через TTS. Ошибка не подсвечивает правильную карточку и просит проверить оба признака ещё раз.

Самостоятельная формулировка для меню: выбери цвет и форму вместе.

## Игровой цикл

```text
озвучивание цвета и формы -> показ крупных карточек -> выбор ответа крупной целью -> проверка -> следующий пример
```
